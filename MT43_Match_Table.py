# -*- coding: utf-8 -*-

#  File Name: MT43_Match_Table.py
# -----------------------------------------------------------------------------
#  Matching tables for MT43
#
#  Copyright (c) 2024 Allystar Technology Co. Limited
#
#  SPDX-License-Identifier: MIT
# -----------------------------------------------------------------------------

matchtable_mt43_Co = { #6
    0: "なし",
    101: "今後若干の海面変動があるかもしれません。",
    102: "今後若干の海面変動があるかもしれませんが、被害の心配はありません。",
    103: "今後もしばらく海面変動が続くと思われます。",
    104: "今後もしばらく海面変動が続くと思われますので、海水浴や磯釣り等を行う際は注意してください。",
    105: "今後もしばらく海面変動が続くと思われますので、磯釣り等を行う際は注意してください。",
    107: "現在、大津波警報・津波警報・津波注意報を発表している沿岸はありません。",
    109: "津波と満潮が重なると、津波はより高くなりますので一層厳重な警戒が必要です。",
    110: "津波と満潮が重なると、津波はより高くなりますので十分な注意が必要です。",
    111: "場所によっては、観測した津波の高さよりさらに大きな津波が到達しているおそれがあります。",
    112: "今後、津波の高さは更に高くなることも考えられます。",
    113: "沖合での観測値をもとに津波が推定されている沿岸では、早いところでは、既に津波が到達していると推定されます。",
    114: "津波による潮位変化が観測されてから最大波が観測されるまでに数時間以上かかることがあります。",
    115: "沖合での観測値であり、沿岸では津波はさらに高くなります。",
    121: "＜大津波警報＞\n大きな津波が襲い甚大な被害が発生します。\n沿岸部や川沿いにいる人はただちに高台や避難ビルなど安全な場所へ避難してください。\n津波は繰り返し襲ってきます。警報が解除されるまで安全な場所から離れないでください。",
    122: "＜津波警報＞\n津波による被害が発生します。\n沿岸部や川沿いにいる人はただちに高台や避難ビルなど安全な場所へ避難してください。\n津波は繰り返し襲ってきます。警報が解除されるまで安全な場所から離れないでください。",
    123: "＜津波注意報＞\n海の中や海岸付近は危険です。\n海の中にいる人はただちに海から上がって、海岸から離れてください。\n潮の流れが速い状態が続きますので、注意報が解除されるまで海に入ったり海岸に近づいたりしないようにしてください。",
    124: "＜津波予報(若干の海面変動)＞\n若干の海面変動が予想されますが、被害の心配はありません。",
    131: "警報が発表された沿岸部や川沿いにいる人はただちに高台や避難ビルなど安全な場所へ避難してください。\n到達予想時刻は、予報区のなかで最も早く津波が到達する時刻です。場所によっては、この時刻よりもかなり遅れて津波が襲ってくることがあります。\n到達予想時刻から津波が最も高くなるまでに数時間以上かかることがありますので、観測された津波の高さにかかわらず、警報が解除されるまで安全な場所から離れないでください。",
    132: "場所によっては津波の高さが「予想される津波の高さ」より高くなる可能性があります。",
    141: "東日本大震災クラスの津波が来襲します。",
    142: "沖合で高い津波を観測したため大津波警報・津波警報に切り替えました。",
    143: "沖合で高い津波を観測したため大津波警報・津波警報を切り替えました。",
    144: "沖合で高い津波を観測したため大津波警報に切り替えました。",
    145: "沖合で高い津波を観測したため大津波警報を切り替えました。",
    146: "沖合で高い津波を観測したため津波警報に切り替えました。",
    147: "沖合で高い津波を観測したため津波警報を切り替えました。",
    148: "沖合で高い津波を観測したため予想される津波の高さを切り替えました。",
    149: "ただちに避難してください。",
    201: "強い揺れに警戒してください。",
    211: "津波警報等(大津波警報・津波警報あるいは津波注意報)を発表中です。",
    212: "この地震により、日本の沿岸では若干の海面変動があるかもしれませんが、被害の心配はありません。",
    213: "今後もしばらく海面変動が続くと思われますので、海水浴や磯釣り等を行う際は注意してください。",
    214: "今後もしばらく海面変動が続くと思われますので、磯釣り等を行う際は注意してください。",
    215: "この地震による津波の心配はありません。",
    216: "震源が海底の場合、津波が発生するおそれがあります。",
    217: "今後の情報に注意してください。",
    221: "太平洋の広域に津波発生の可能性があります。",
    222: "太平洋で津波発生の可能性があります。",
    223: "北西太平洋で津波発生の可能性があります。",
    224: "インド洋の広域に津波発生の可能性があります。",
    225: "インド洋で津波発生の可能性があります。",
    226: "震源の近傍で津波発生の可能性があります。",
    227: "震源の近傍で小さな津波発生の可能性がありますが、被害をもたらす津波の心配はありません。",
    228: "一般的に、この規模の地震が海域の浅い領域で発生すると、津波が発生することがあります。",
    229: "日本への津波の有無については現在調査中です。",
    230: "この地震による日本への津波の影響はありません。",
    241: "この地震について、緊急地震速報を発表しています。",
    242: "この地震について、緊急地震速報を発表しています。この地震の最大震度は2でした。",
    243: "この地震について、緊急地震速報を発表しています。この地震の最大震度は1でした。",
    244: "この地震について、緊急地震速報を発表しています。この地震で震度1以上は観測されていません。",
    245: "この地震で緊急地震速報を発表しましたが、強い揺れは観測されませんでした。",
    256: "震源要素を訂正します。",
    500: "その他の防災上の留意事項"   
}

def get_mt43_Co(Co):
    Co_dec = matchtable_mt43_Co.get(Co, "防災上の留意事項(コード番号：NNN)" )
    return Co_dec
    
matchtable_mt43_Ep = { #7
    11: "北海道地方",
    12: "東北地方",
    13: "北陸地方",
    14: "関東甲信地方",
    15: "小笠原地方",
    16: "東海地方",
    17: "近畿地方",
    18: "中国地方",
    19: "四国地方",
    20: "九州地方",
    21: "沖縄地方",
    100: "石狩地方北部",
    101: "石狩地方中部",
    102: "石狩地方南部",
    105: "渡島地方北部",
    106: "渡島地方東部",
    107: "渡島地方西部",
    110: "檜山地方",
    115: "後志地方北部",
    116: "後志地方東部",
    117: "後志地方西部",
    120: "空知地方北部",
    121: "空知地方中部",
    122: "空知地方南部",
    125: "上川地方北部",
    126: "上川地方中部",
    127: "上川地方南部",
    130: "留萌地方中北部",
    131: "留萌地方南部",
    135: "宗谷地方北部",
    136: "宗谷地方南部",
    140: "網走地方",
    141: "北見地方",
    142: "紋別地方",
    145: "胆振地方西部",
    146: "胆振地方中東部",
    150: "日高地方西部",
    151: "日高地方中部",
    152: "日高地方東部",
    155: "十勝地方北部",
    156: "十勝地方中部",
    157: "十勝地方南部",
    160: "釧路地方北部",
    161: "釧路地方中南部",
    165: "根室地方北部",
    166: "根室地方中部",
    167: "根室地方南部",
    180: "北海道南西沖",
    181: "北海道西方沖",
    182: "石狩湾",
    183: "北海道北西沖",
    184: "宗谷海峡",
    186: "国後島付近",
    187: "択捉島付近",
    188: "北海道東方沖",
    189: "根室半島南東沖",
    190: "釧路沖",
    191: "十勝沖",
    192: "浦河沖",
    193: "苫小牧沖",
    194: "内浦湾",
    195: "宗谷東方沖",
    196: "網走沖",
    197: "択捉島南東沖",
    200: "青森県津軽北部",
    201: "青森県津軽南部",
    202: "青森県三八上北地方",
    203: "青森県下北地方",
    210: "岩手県沿岸北部",
    211: "岩手県沿岸南部",
    212: "岩手県内陸北部",
    213: "岩手県内陸南部",
    220: "宮城県北部",
    221: "宮城県南部",
    222: "宮城県中部",
    230: "秋田県沿岸北部",
    231: "秋田県沿岸南部",
    232: "秋田県内陸北部",
    233: "秋田県内陸南部",
    240: "山形県庄内地方",
    241: "山形県最上地方",
    242: "山形県村山地方",
    243: "山形県置賜地方",
    250: "福島県中通り",
    251: "福島県浜通り",
    252: "福島県会津",
    280: "津軽海峡",
    281: "山形県沖",
    282: "秋田県沖",
    283: "青森県西方沖",
    284: "陸奥湾",
    285: "青森県東方沖",
    286: "岩手県沖",
    287: "宮城県沖",
    288: "三陸沖",
    289: "福島県沖",
    300: "茨城県北部",
    301: "茨城県南部",
    309: "千葉県南東沖",
    310: "栃木県北部",
    311: "栃木県南部",
    320: "群馬県北部",
    321: "群馬県南部",
    330: "埼玉県北部",
    331: "埼玉県南部",
    332: "埼玉県秩父地方",
    340: "千葉県北東部",
    341: "千葉県北西部",
    342: '千葉県南部',
    349: '房総半島南方沖',
    350: '東京都２３区',
    351: '東京都多摩東部',
    352: '東京都多摩西部',
    360: '神奈川県東部',
    361: '神奈川県西部',
    370: '新潟県上越地方',
    371: '新潟県中越地方',
    372: '新潟県下越地方',
    378: '新潟県下越沖',
    379: '新潟県上中越沖',
    380: '富山県東部',
    381: '富山県西部',
    390: '石川県能登地方',
    391: '石川県加賀地方',
    400: '福井県嶺北',
    401: '福井県嶺南',
    411: '山梨県中・西部',
    412: '山梨県東部・富士五湖',
    420: '長野県北部',
    421: '長野県中部',
    422: '長野県南部',
    430: '岐阜県飛騨地方',
    431: '岐阜県美濃東部',
    432: '岐阜県美濃中西部',
    440: '静岡県伊豆地方',
    441: '静岡県東部',
    442: '静岡県中部',
    443: '静岡県西部',
    450: '愛知県東部',
    451: '愛知県西部',
    460: '三重県北部',
    461: '三重県中部',
    462: '三重県南部',
    469: '三重県南東沖',
    471: '茨城県沖',
    472: '関東東方沖',
    473: '千葉県東方沖',
    475: '八丈島東方沖',
    476: '八丈島近海',
    477: '東京湾',
    478: '相模湾',
    480: '伊豆大島近海',
    481: '伊豆半島東方沖',
    482: '三宅島近海',
    483: '新島・神津島近海',
    485: '駿河湾',
    486: '駿河湾南方沖',
    489: '三河湾',
    490: '伊勢湾',
    492: '若狭湾',
    493: '福井県沖',
    494: '石川県西方沖',
    495: '能登半島沖',
    497: '富山湾',
    498: '佐渡付近',
    499: '東海道南方沖',
    500: '滋賀県北部',
    501: '滋賀県南部',
    510: '京都府北部',
    511: '京都府南部',
    520: '大阪府北部',
    521: '大阪府南部',
    530: '兵庫県北部',
    531: '兵庫県南東部',
    532: '兵庫県南西部',
    540: '奈良県',
    550: '和歌山県北部',
    551: '和歌山県南部',
    560: '鳥取県東部',
    562: '鳥取県中部',
    563: '鳥取県西部',
    570: '島根県東部',
    571: '島根県西部',
    580: '岡山県北部',
    581: '岡山県南部',
    590: '広島県北部',
    591: '広島県南東部',
    592: '広島県南西部',
    600: '徳島県北部',
    601: '徳島県南部',
    610: '香川県東部',
    611: '香川県西部',
    620: '愛媛県東予',
    621: '愛媛県中予',
    622: '愛媛県南予',
    630: '高知県東部',
    631: '高知県中部',
    632: '高知県西部',
    673: '土佐湾',
    674: '紀伊水道',
    675: '大阪湾',
    676: '播磨灘',
    677: '瀬戸内海中部',
    678: '安芸灘',
    679: '周防灘',
    680: '伊予灘',
    681: '豊後水道',
    682: '山口県北西沖',
    683: '島根県沖',
    684: '鳥取県沖',
    685: '隠岐島近海',
    686: '兵庫県北方沖',
    687: '京都府沖',
    688: '淡路島付近',
    689: '和歌山県南方沖',
    700: '山口県北部',
    702: '山口県西部',
    703: '山口県東部',
    704: '山口県中部',
    710: '福岡県福岡地方',
    711: '福岡県北九州地方',
    712: '福岡県筑豊地方',
    713: '福岡県筑後地方',
    720: '佐賀県北部',
    721: '佐賀県南部',
    730: '長崎県北部',
    731: '長崎県南西部',
    732: '長崎県島原半島',
    740: '熊本県阿蘇地方',
    741: '熊本県熊本地方',
    742: '熊本県球磨地方',
    743: '熊本県天草・芦北地方',
    750: '大分県北部',
    751: '大分県中部',
    752: '大分県南部',
    753: '大分県西部',
    760: '宮崎県北部平野部',
    761: '宮崎県北部山沿い',
    762: '宮崎県南部平野部',
    763: '宮崎県南部山沿い',
    770: '鹿児島県薩摩地方',
    771: '鹿児島県大隅地方',
    783: '五島列島近海',
    784: '天草灘',
    785: '有明海',
    786: '橘湾',
    787: '鹿児島湾',
    790: '種子島近海',
    791: '日向灘',
    793: '奄美大島近海',
    795: '壱岐・対馬近海',
    796: '福岡県北西沖',
    797: '薩摩半島西方沖',
    798: 'トカラ列島近海',
    799: '奄美大島北西沖',
    820: '大隅半島東方沖',
    821: '九州地方南東沖',
    822: '種子島南東沖',
    823: '奄美大島北東沖',
    850: '沖縄本島近海',
    851: '南大東島近海',
    852: '沖縄本島南方沖',
    853: '宮古島近海',
    854: '石垣島近海',
    855: '石垣島南方沖',
    856: '西表島付近',
    857: '与那国島近海',
    858: '沖縄本島北西沖',
    859: '宮古島北西沖',
    860: '石垣島北西沖',
    900: '台湾付近',
    901: '東シナ海',
    902: '四国沖',
    903: '鳥島近海',
    904: '鳥島東方沖',
    905: 'オホーツク海南部',
    906: 'サハリン西方沖',
    907: '日本海北部',
    908: '日本海中部',
    909: '日本海西部',
    911: '父島近海',
    912: '千島列島',
    913: '千島列島南東沖',
    914: '北海道南東沖',
    915: '東北地方東方沖',
    916: '小笠原諸島西方沖',
    917: '硫黄島近海',
    918: '小笠原諸島東方沖',
    919: '南海道南方沖',
    920: '薩南諸島東方沖',
    921: '本州南方沖',
    922: 'サハリン南部付近',
    930: '北西太平洋',
    932: 'マリアナ諸島',
    933: '黄海',
    934: '朝鮮半島南部',
    935: '朝鮮半島北部',
    936: '中国東北部',
    937: 'ウラジオストク付近',
    938: 'シベリア南部',
    939: 'サハリン近海',
    940: 'アリューシャン列島',
    941: 'カムチャツカ半島付近',
    942: '北米西部',
    943: '北米中部',
    944: '北米東部',
    945: '中米',
    946: '南米西部',
    947: '南米中部',
    948: '南米東部',
    949: '北東太平洋',
    950: '南太平洋',
    951: 'インドシナ半島付近',
    952: 'フィリピン付近',
    953: 'インドネシア付近',
    954: 'グアム付近',
    955: 'ニューギニア付近',
    956: 'ニュージーランド付近',
    957: 'オーストラリア付近',
    958: 'シベリア付近',
    964: '中国中部',
    965: '中国東部',
    966: 'インド付近',
    967: 'インド洋',
    968: '中東',
    969: 'ヨーロッパ西部',
    970: 'ヨーロッパ中部',
    971: 'ヨーロッパ東部',
    972: '地中海',
    973: 'アフリカ西部',
    974: 'アフリカ中部',
    975: 'アフリカ東部',
    976: '北大西洋',
    977: '南大西洋',
    978: '北極付近',
    979: '南極付近',
    999: '遠地',
    1000: 'その他の震央地名'    
}

def get_mt43_Ep(Ep):
    Ep_dec = matchtable_mt43_Ep.get(Ep, "震央地名(コード番号：NNN)")
    return Ep_dec
    
matchtable_mt43_earthquakeearly_L1 = { #8
    1: '震度 0',
    2: '震度 1',
    3: '震度 2',
    4: '震度 3',
    5: '震度 4',
    6: '震度 5 弱',
    7: '震度 5 強',
    8: '震度 6 弱',
    9: '震度 6 強',
    10: '震度 7',
    14: 'なし',
    15: '不明'
}

def get_mt43_earthquakeearly_L1(L1):
    L1_dec = matchtable_mt43_earthquakeearly_L1.get(L1, "")
    return L1_dec 

matchtable_mt43_earthquakeearly_U1 = { #9
    1: '震度 0',
    2: '震度 1',
    3: '震度 2',
    4: '震度 3',
    5: '震度 4',
    6: '震度 5 弱',
    7: '震度 5 強',
    8: '震度 6 弱',
    9: '震度 6 強',
    10: '震度 7',
    11: '～程度以上',
    14: 'なし',
    15: '不明'
}

def get_mt43_earthquakeearly_U1(U1):
    U1_dec = matchtable_mt43_earthquakeearly_U1.get(U1,"")
    return U1_dec

matchtable_mt43_earthquakeearly_Pl = { #10
    1: '北海道道央',
    2: '北海道道南',
    3: '北海道道北',
    4: '北海道道東',
    5: '青森',
    6: '岩手',
    7: '宮城',
    8: '秋田',
    9: '山形',
    10: '福島',
    11: '茨城',
    12: '栃木',
    13: '群馬',
    14: '埼玉',
    15: '千葉',
    16: '東京',
    17: '伊豆諸島',
    18: '小笠原',
    19: '神奈川',
    20: '新潟',
    21: '富山',
    22: '石川',
    23: '福井',
    24: '山梨',
    25: '長野',
    26: '岐阜',
    27: '静岡',
    28: '愛知',
    29: '三重',
    30: '滋賀',
    31: '京都',
    32: '大阪',
    33: '兵庫',
    34: '奈良',
    35: '和歌山',
    36: '鳥取',
    37: '島根',
    38: '岡山',
    39: '広島',
    40: '山口',
    41: '徳島',
    42: '香川',
    43: '愛媛',
    44: '高知',
    45: '福岡',
    46: '佐賀',
    47: '長崎',
    48: '熊本',
    49: '大分',
    50: '宮崎',
    51: '鹿児島',
    52: '奄美(群島)',
    53: '沖縄本島',
    54: '大東島',
    55: '宮古島',
    56: '八重山',
    57: '北海道',
    58: '東北',
    59: '関東',
    60: '伊豆諸島',
    61: '小笠原',
    62: '北陸',
    63: '甲信',
    64: '東海',
    65: '近畿',
    66: '中国',
    67: '四国',
    68: '九州',
    69: '奄美(群島)',
    70: '沖縄',
    80: 'その他の府県予報区および地方予報区_緊急地震速報(警報)'
}

def get_mt43_earthquakeearly_Pl(Pl):
    Pl_dec = matchtable_mt43_earthquakeearly_Pl.get(Pl, '府県予報区および地方予報区_緊急地震速報(警報)(地点：NN)')
    return Pl_dec

matchtable_mt43_earthquakeearly_LgL1 = { #11-1
    1: '長周期地震動階級 1 未満',
    2: '長周期地震動階級 1',
    3: '長周期地震動階級 2',
    4: '長周期地震動階級 3',
    5: '長周期地震動階級 4',
    7: '不明'
}

def get_mt43_earthquakeearly_LgL1(LgL1):
    LgL1_dec = matchtable_mt43_earthquakeearly_LgL1.get(LgL1, "")
    return LgL1_dec

matchtable_mt43_earthquakeearly_LgU1 = { #11-2
    1: '長周期地震動階級 1 未満',
    2: '長周期地震動階級 1',
    3: '長周期地震動階級 2',
    4: '長周期地震動階級 3',
    5: '長周期地震動階級 4',
    6: '～程度以上',
    7: '不明'
}

def get_mt43_earthquakeearly_LgU1(LgU1):
    LgU1_dec = matchtable_mt43_earthquakeearly_LgU1.get(LgU1, "")
    return LgU1_dec

#----------
matchtable_mt43_seismic_Es = { #15
    1: '震度 4 未満*',
    2: '震度 4',
    3: '震度 5 弱',
    4: '震度 5 強',
    5: '震度 6 弱',
    6: '震度 6 強',
    7: '震度 7'
}

def get_mt43_seismic_Es(Es):
    Es_dec = matchtable_mt43_seismic_Es.get(Es, "")
    return Es_dec

matchtable_mt43_seismic_Pl = { #16
    1: '北海道',
    2: '青森県',
    3: '岩手県',
    4: '宮城県',
    5: '秋田県',
    6: '山形県',
    7: '福島県',
    8: '茨城県',
    9: '栃木県',
    10: '群馬県',
    11: '埼玉県',
    12: '千葉県',
    13: '東京都',
    14: '神奈川県',
    15: '新潟県',
    16: '富山県',
    17: '石川県',
    18: '福井県',
    19: '山梨県',
    20: '長野県',
    21: '岐阜県',
    22: '静岡県',
    23: '愛知県',
    24: '三重県',
    25: '滋賀県',
    26: '京都府',
    27: '大阪府',
    28: '兵庫県',
    29: '奈良県',
    30: '和歌山県',
    31: '鳥取県',
    32: '島根県',
    33: '岡山県',
    34: '広島県',
    35: '山口県',
    36: '徳島県',
    37: '香川県',
    38: '愛媛県',
    39: '高知県',
    40: '福岡県',
    41: '佐賀県',
    42: '長崎県',
    43: '熊本県',
    44: '大分県',
    45: '宮崎県',
    46: '鹿児島県',
    47: '沖縄県'
}

def get_mt43_seismic_Pl(Pl):
    Pl_dec = matchtable_mt43_seismic_Pl.get(Pl, "")
    return Pl_dec

#----------
matchtable_mt43_nankai_Is = { #19
    1: '調査中 A',
    2: '調査中 B',
    3: '調査中 C',
    4: '巨大地震警戒',
    5: '巨大地震注意',
    6: '調査終了',
}

def get_mt43_nankai_Is(Is):
    Is_dec = matchtable_mt43_nankai_Is.get(Is, 'その他')
    return Is_dec

#----------
matchtable_mt43_tsunami_Dw = { #22
    1: '津波なし',
    2: '警報解除',
    3: '津波警報',
    4: '大津波警報',
    5: '大津波警報：発表',
    15: 'その他の警報'                                                         
}

def get_mt43_tsunami_Dw(Dw):
    Dw_dec = matchtable_mt43_tsunami_Dw.get(Dw, "警報(コード番号：NN)")
    return Dw_dec    

matchtable_mt43_tsunami_Th = { #23
    1: '0.2m 未満',
    2: '1m',
    3: '3m',
    4: '5m',
    5: '10m',
    6: '10m 超',
    14: '不明',
    15: 'その他の津波の高さ'
}

def get_mt43_tsunami_Th(Th):
    Th_dec = matchtable_mt43_tsunami_Th.get(Th, "津波の高さ(コード番号：NN)")
    return Th_dec

matchtable_mt43_tsunami_Pl = { #24
    #: *2 are the codes that not currently transmitted as JMA-DC Report.                           
    100: "北海道太平洋沿岸東部",
    101: "北海道太平洋沿岸中部",
    102: "北海道太平洋沿岸西部",
    110: "北海道日本海沿岸北部",
    111: "北海道日本海沿岸南部",
    120: "オホーツク海沿岸",
    191: "北海道太平洋沿岸",  #: *2
    192: "北海道日本海沿岸",  #: *2
    193: "Reserved",
    200: "青森県日本海沿岸",
    201: "青森県太平洋沿岸",
    202: "陸奥湾",
    210: "岩手県",
    220: "宮城県",
    230: "秋田県",
    240: "山形県",
    250: "福島県",
    281: "青森県",  #: *2
    291: "東北地方太平洋沿岸",  #: *2
    292: "東北地方日本海沿岸",  #: *2
    300: "茨城県",
    310: "千葉県九十九里・外房",
    311: "千葉県内房",
    312: "東京湾内湾",
    320: "伊豆諸島",
    321: "小笠原諸島",
    330: "相模湾・三浦半島",
    340: "新潟県上中下越",
    341: "佐渡",
    350: "富山県",
    360: "石川県能登",
    361: "石川県加賀",
    370: "福井県",
    380: "静岡県",
    390: "愛知県外海",
    391: "伊勢・三河湾",
    400: "三重県南部",
    481: "千葉県",  #: *2
    482: "神奈川県",  #: *2
    483: "新潟県",  #: *2
    484: "石川県",  #: *2
    485: "愛知県",  #: *2
    486: "三重県",  #: *2
    491: "関東地方",  #: *2
    492: "伊豆・小笠原諸島",  #: *2
    493: "北陸地方",  #: *2
    494: "東海地方",  #: *2
    500: "京都府",
    510: "大阪府",
    520: "兵庫県北部",
    521: "兵庫県瀬戸内海沿岸",
    522: "淡路島南部",
    530: "和歌山県",
    540: "鳥取県",
    550: "島根県出雲・石見",
    551: "隠岐",
    560: "岡山県",
    570: "広島県",
    580: "徳島県",
    590: "香川県",
    600: '愛媛県宇和海沿岸',
    601: '愛媛県瀬戸内海沿岸',
    610: '高知県',
    681: '兵庫県',  # *2
    682: '島根県',  # *2
    683: '愛媛県',  # *2
    691: '近畿四国太平洋沿岸',  # *2
    692: '近畿中国日本海沿岸',  # *2
    693: '瀬戸内海沿岸2',
    700: '山口県日本海沿岸',
    701: '山口県瀬戸内海沿岸',
    710: '福岡県瀬戸内海沿岸',
    711: '福岡県日本海沿岸',
    712: '有明・八代海',
    720: '佐賀県北部',
    730: '長崎県西方',
    731: '壱岐・対馬',
    740: '熊本県天草灘沿岸',
    750: '大分県瀬戸内海沿岸',
    751: '大分県豊後水道沿岸',
    760: '宮崎県',
    770: '鹿児島県東部',
    771: '種子島・屋久島地方',
    772: '奄美群島・トカラ列島',
    773: '鹿児島県西部',
    781: '山口県',  # *2
    782: '福岡県',  # *2
    783: '佐賀県',  # *2
    784: '長崎県',  # *2
    785: '熊本県',  # *2
    786: '大分県',  # *2
    787: '鹿児島県',  # *2
    791: '九州地方東部',  # *2
    792: '九州地方西部',  # *2
    793: '薩南諸島',  # *2
    800: '沖縄本島地方',
    801: '大東島地方',
    802: '宮古島・八重山地方',
    891: '沖縄県地方',  # *2
    990: 'Reserved',
    1000: 'その他の津波予報区'                              
}

def get_mt43_tsunami_Pl(Pl):
    Pl_dec = matchtable_mt43_tsunami_Pl.get(Pl, "津波予報区(コード番号：NNN)")
    return Pl_dec

#----------
matchtable_mt43_nwptsunami_Tp = { #27
    0: "THERE IS NO POSSIBILITY OF A TSUNAMI",
    1: "THERE IS A POSSIBILITY OF A DESTRUCTIVE OCEAN-WIDE TSUNAMI",
    2: "THERE IS A POSSIBILITY OF A DESTRUCTIVE REGIONAL TSUNAMI",
    3: "THERE IS A POSSIBILITY OF A DESTRUCTIVE LOCAL TSUNAMI NEAR THE EPICENTER",
    4: "THERE IS A VERY SMALL POSSIBILITY OF A DESTRUCTIVE LOCAL TSUNAMI",
    7: "その他の津波発生の可能性有無"                     
}

def get_mt43_nwptsunami_Tp(Tp):
    Tp_dec = matchtable_mt43_nwptsunami_Tp.get(Tp, "津波発生の可能性有無(コード番号：N)")
    return Tp_dec

matchtable_mt43_nwptsunami_Th = { #27a
    1: "0.3m~1m",
    2: "1m~3m",
    3: "3m~5m",
    4: "5m~10m",
    508: "10m 超",
    509: "巨大",
    510: "高い",
    511: "不明"                                                                  
}

def get_mt43_nwptsunami_Th(Th):
    Th_dec = matchtable_mt43_nwptsunami_Th(Th, "")
    return Th_dec

matchtable_mt43_nwptsunami_Pl = { #28
    1: "Ust-Kamchatsk (East Coasts of Kamchatka Peninsula)",
    2: "Petropavlovsk-K (East Coasts of Kamchatka Peninsula)",
    3: "Severo Kurilsk (Kuril Islands)",
    4: "Urup Islands (Kuril Islands)",
    5: "Busan (South Coasts of Korean Peninsula)",
    6: "Nohwa (South Coasts of Korean Peninsula)",
    7: "Seogwipo (South Coasts of Korean Peninsula)",
    8: "Hualien (Taiwan)",
    9: "Basco (East Coasts of Philippines)",
    10: "Palanan (East Coasts of Philippines)",
    11: "Legaspi (East Coasts of Philippines)",
    12: "Laoang (East Coasts of Philippines)",
    13: "Madrid (East Coasts of Philippines)",
    14: "Davao (East Coasts of Philippines)",
    15: "Berebere (North Coasts of Irian Jaya)",
    16: "Patani (North Coasts of Irian Jaya)",
    17: "Sorong (North Coasts of Irian Jaya)",
    18: "Manokwari (North Coasts of Irian Jaya)",
    19: "Warsa (North Coasts of Irian Jaya)",
    20: "Jayapura (North Coasts of Irian Jaya)",
    21: "Vanimo (North Coasts of Papua New Guinea)",
    22: "Wewak (North Coasts of Papua New Guinea)",
    23: "Madang (North Coasts of Papua New Guinea)",
    24: "Manus Islands (North Coasts of Papua New Guinea)",
    25: "Rabaul (North Coasts of Papua New Guinea)",
    26: "Kavieng (North Coasts of Papua New Guinea)",
    27: "Kimbe (North Coasts of Papua New Guinea)",
    28: "Kieta (North Coasts of Papua New Guinea)",
    29: "Guam (Mariana Islands)",
    30: "Saipan (Mariana Islands)",
    31: "Malakal (Palau)",
    32: "Yap Island (Micronesia)",
    33: "Chuuk Island (Micronesia)",
    34: "Pohnpei Island (Micronesia)",
    35: "Kosrae Island (Micronesia)",
    36: "Eniwetok Island (Marshall Islands)",
    37: "Panggoe (North Coasts of Solomon Islands)",
    38: "Auki (North Coasts of Solomon Islands)",
    39: "Kirakira (North Coasts of Solomon Islands)",
    40: "Munda (Solomon Sea)",
    41: "Honiara (Solomon Sea)",
    42: "Reserved",
    43: "Reserved",
    44: "Reserved",
    45: "Reserved",
    46: "Reserved",
    47: "Reserved",
    48: "Reserved",
    49: "Reserved",
    50: "Reserved",
    51: "Reserved",
    52: "Reserved",
    53: "Reserved",
    54: "Reserved",
    55: "Reserved",
    56: "Reserved",
    57: "Reserved",
    58: "Reserved",
    59: "Reserved",
    60: "Reserved",
    61: "Reserved",
    62: "Reserved",
    63: "Reserved",
    64: "Reserved",
    65: "Reserved",
    66: "Ostrov-Karaginskiy (East Coasts of Kamchatka Peninsula)",
    67: "Nikolskoya (East Coasts of Kamchatka Peninsula)",
    68: "Tongyeong (South Coasts of Korean Peninsula)",
    69: "Heuksando (South Coasts of Korean Peninsula)",
    70: "Cheju-Island (South Coasts of Korean Peninsula)",
    71: "Chilung (Taiwan)",
    72: "Taitung (Taiwan)",
    73: "Reserved",
    74: "Homel (Taiwan)",
    75: "Geme (North Coasts of Irian Jaya)",
    76: "Ulamona (North Coasts of Papua New Guinea)",
    77: "Ghatere (North Coasts of Solomon Islands)",
    78: "Amun (Solomon Sea)",
    79: "Falamae (Solomon Sea)",
    80: "Misima (Solomon Sea)",
    81: "Alotau (Solomon Sea)",
    82: "Lae (Solomon Sea)",
    83: "Port-Moresby (Coral Sea)",
    84: "Shanghai (Coasts of East China Sea)",
    85: "Zhoushan (Coasts of East China Sea)",
    86: "Wenzhou (Coasts of East China Sea)",
    87: "Reserved",
    88: "Reserved",
    89: "Reserved",
    90: "Reserved",
    91: "Reserved",
    92: "Reserved",
    93: "Reserved",
    94: "Reserved",
    95: "Reserved",
    96: "Reserved",
    99: "Unknown",
    100: "Other region"                                 
}

def get_mt43_nwptsunami_Pl(Pl):
    Pl_dec = matchtable_mt43_nwptsunami_Pl.get(Pl, "Coastal Region(Code:NN)")
    return Pl_dec
#----------
matchtable_mt43_volcano_Dw = { #31
    1: "Reserved",
    2: "Reserved",
    3: "Reserved",
    4: "Reserved",
    5: "Reserved",
    11: "レベル 1(活火山であることに留意)",
    12: "レベル 2(火口周辺規制)",
    13: "レベル 3(入山規制)",
    14: "レベル 4(高齢者等避難)",
    15: "レベル 5(避難)",
    21: "活火山であることに留意",
    22: "火口周辺危険",
    23: "入山危険",
    24: "山麓厳重警戒",
    25: "居住地域厳重警戒",
    31: "Reserved",
    32: "Reserved",
    33: "Reserved",
    35: "活火山であることに留意(海底火山)",
    36: "周辺海域警戒",
    41: "Reserved",
    42: "Reserved",
    43: "Reserved",
    44: "Reserved",
    45: "Reserved",
    46: "Reserved",
    47: "Reserved",
    48: "Reserved",
    49: "Reserved",
    51: "Reserved",
    52: "噴火",
    53: "Reserved",
    54: "Reserved",
    55: "Reserved",
    56: "Reserved",
    61: "Reserved",
    62: "噴火したもよう",
    63: "Reserved",
    64: "Reserved",
    65: "Reserved",
    91: "Reserved",
    99: "Reserved",
    127: "その他の防災気象情報要素"    
}

def get_mt43_volcano_Dw(Dw):
    Dw_dec = matchtable_mt43_volcano_Dw.get(Dw, "NNN* 防災気象情報要素(コード番号：NNN)")
    return Dw_dec

matchtable_mt43_volcano_Vo = { #32
    101: '知床硫黄山',
    102: '羅臼岳',
    103: '摩周',
    104: 'アトサヌプリ',
    105: '雌阿寒岳',
    106: '丸山',
    107: '大雪山',
    108: '十勝岳',
    109: '樽前山',
    110: '恵庭岳',
    111: '倶多楽',
    112: '有珠山',
    113: '北海道駒ヶ岳',
    114: '恵山',
    115: '渡島大島',
    116: '利尻山',
    117: '羊蹄山',
    118: 'ニセコ',
    119: '天頂山',
    120: '雄阿寒岳',
    151: '茂世路岳',
    152: '散布山',
    153: '指臼岳',
    154: '小田萌山',
    155: '択捉焼山',
    156: '択捉阿登佐岳',
    157: 'ベルタルベ山',
    158: '爺爺岳',
    159: '羅臼山',
    160: '泊山',
    161: 'ルルイ岳',
    201: '恐山',
    202: '岩木山',
    203: '八甲田山',
    204: '十和田',
    205: '秋田焼山',
    206: '八幡平',
    207: '岩手山',
    208: '秋田駒ヶ岳',
    209: '鳥海山',
    210: '栗駒山',
    211: '鳴子',
    212: '蔵王山',
    213: '吾妻山',
    214: '安達太良山',
    215: '磐梯山',
    216: '燧ヶ岳',
    217: '肘折',
    218: '沼沢',
    301: '那須岳',
    302: '日光白根山',
    303: '赤城山',
    304: '榛名山',
    305: '草津白根山',
    306: '浅間山',
    307: '新潟焼山',
    308: '妙高山',
    309: '弥陀ヶ原',
    310: '焼岳',
    311: '乗鞍岳',
    312: '御嶽山',
    313: '白山',
    314: '富士山',
    315: '箱根山',
    316: '伊豆東部火山群',
    317: '伊豆大島',
    318: '新島',
    319: '神津島',
    320: '三宅島',
    321: '八丈島',
    322: '青ヶ島',
    323: 'ベヨネース列岩',
    324: '須美寿島',
    325: '伊豆鳥島',
    326: '西之島',
    327: '海徳海山',
    328: '噴火浅根',
    329: '硫黄島',
    330: '北福徳堆',
    331: '福徳岡ノ場',
    333: '高原山',
    334: '横岳',
    335: 'アカンダナ山',
    336: '利島',
    337: '御蔵島',
    338: '孀婦岩',
    339: '海形海山',
    340: '南日吉海山',
    341: '日光海山',
    342: '男体山',
    350: '草津白根山(白根山(湯釜付近))',
    351: '草津白根山(本白根山)',
    401: '三瓶山',
    502: '九重山',
    503: '阿蘇山',
    504: '雲仙岳',
    505: '霧島山',
    506: '桜島',
    507: '開聞岳',
    508: '薩摩硫黄島',
    509: '口永良部島',
    510: '中之島',
    511: '諏訪之瀬島',
    512: '阿武火山群',
    513: '鶴見岳・伽藍岳',
    514: '由布岳',
    515: '福江火山群',
    516: '米丸・住吉池',
    517: '若尊',
    518: '池田・山川',
    519: '口之島',
    550: '霧島山(御鉢)',
    551: '霧島山(新燃岳)',
    552: '霧島山(えびの高原(硫黄山)周辺)',
    553: '霧島山(大幡池)',
    601: '硫黄鳥島',
    602: '西表島北北東海底火山',
    900: '全国の活火山',
    901: 'その他の活火山',
    902: '新たな活火山',
    4000: 'その他の火山'    
}

def get_mt43_volcano_Vo(Vo):
    Vo_dec = matchtable_mt43_volcano_Vo.get(Vo, " 火山(コード番号：NNNN)")
    return Vo_dec

matchtable_mt43_volcano_Pl = { #33 very long one, later add
	110000: "北海道札幌市",
	120200: "北海道函館市",
	120300: "北海道小樽市",
	120400: "北海道旭川市",
	120500: "北海道室蘭市",
	120600: "北海道釧路市",
	120700: "北海道帯広市",
	120800: "北海道北見市",
	120900: "北海道夕張市",
	121000: "北海道岩見沢市",
	121100: "北海道網走市",
	121200: "北海道留萌市",
	121300: "北海道苫小牧市",
	121400: "北海道稚内市",
	121500: "北海道美唄市",
	121600: "北海道芦別市",
	121700: "北海道江別市",
	121800: "北海道赤平市",
	121900: "北海道紋別市",
	122000: "北海道士別市",
	122100: "北海道名寄市",
	122200: "北海道三笠市",
	122300: "北海道根室市",
	122400: "北海道千歳市",
	122500: "北海道滝川市",
	122600: "北海道砂川市",
	122700: "北海道歌志内市",
	122800: "北海道深川市",
	122900: "北海道富良野市",
	123000: "北海道登別市",
	123100: "北海道恵庭市",
	123300: "北海道伊達市",
	123400: "北海道北広島市",
	123500: "北海道石狩市",
	123600: "北海道北斗市",
	130300: "北海道当別町",
	130400: "北海道新篠津村",
	133100: "北海道松前町",
	133200: "北海道福島町",
	133300: "北海道知内町",
	133400: "北海道木古内町",
	133700: "北海道七飯町",
	134300: "北海道鹿部町",
	134500: "北海道森町",
	134600: "北海道八雲町",
	134700: "北海道長万部町",
	136100: "北海道江差町",
	136200: "北海道上ノ国町",
	136300: "北海道厚沢部町",
	136400: "北海道乙部町",
	136700: "北海道奥尻町",
	137000: "北海道今金町",
	137100: "北海道せたな町",
	139100: "北海道島牧村",
	139200: "北海道寿都町",
	139300: "北海道黒松内町",
	139400: "北海道蘭越町",
	139500: "北海道ニセコ町",
	139600: "北海道真狩村",
	139700: "北海道留寿都村",
	139800: "北海道喜茂別町",
	139900: "北海道京極町",
	140000: "北海道倶知安町",
	140100: "北海道共和町",
	140200: "北海道岩内町",
	140300: "北海道泊村",
	140400: "北海道神恵内村",
	140500: "北海道積丹町",
	140600: "北海道古平町",
	140700: "北海道仁木町",
	140800: "北海道余市町",
	140900: "北海道赤井川村",
	142300: "北海道南幌町",
	142400: "北海道奈井江町",
	142500: "北海道上砂川町",
	142700: "北海道由仁町",
	142800: "北海道長沼町",
	142900: "北海道栗山町",
	143000: "北海道月形町",
	143100: "北海道浦臼町",
	143200: "北海道新十津川町",
	143300: "北海道妹背牛町",
	143400: "北海道秩父別町",
	143600: "北海道雨竜町",
	143700: "北海道北竜町",
	143800: "北海道沼田町",
	145200: "北海道鷹栖町",
	145300: "北海道東神楽町",
	145400: "北海道当麻町",
	145500: "北海道比布町",
	145600: "北海道愛別町",
	145700: "北海道上川町",
	145800: "北海道東川町",
	145900: "北海道美瑛町",
	146000: "北海道上富良野町",
	146100: "北海道中富良野町",
	146200: "北海道南富良野町",
	146300: "北海道占冠村",
	146400: "北海道和寒町",
	146500: "北海道剣淵町",
	146800: "北海道下川町",
	146900: "北海道美深町",
	147000: "北海道音威子府村",
	147100: "北海道中川町",
	147200: "北海道幌加内町",
	148100: "北海道増毛町",
	148200: "北海道小平町",
	148300: "北海道苫前町",
	148400: "北海道羽幌町",
	148500: "北海道初山別村",
	148600: "北海道遠別町",
	148700: "北海道天塩町",
	151100: "北海道猿払村",
	151200: "北海道浜頓別町",
	151300: "北海道中頓別町",
	151400: "北海道枝幸町",
	151600: "北海道豊富町",
	151700: "北海道礼文町",
	151800: "北海道利尻町",
	151900: "北海道利尻富士町",
	152000: "北海道幌延町",
	154300: "北海道美幌町",
	154400: "北海道津別町",
	154500: "北海道斜里町",
	154600: "北海道清里町",
	154700: "北海道小清水町",
	154900: "北海道訓子府町",
	155000: "北海道置戸町",
	155200: "北海道佐呂間町",
	155500: "北海道遠軽町",
	155900: "北海道湧別町",
	156000: "北海道滝上町",
	156100: "北海道興部町",
	156200: "北海道西興部村",
	156300: "北海道雄武町",
	156400: "北海道大空町",
	157100: "北海道豊浦町",
	157500: "北海道壮瞥町",
	157800: "北海道白老町",
	158100: "北海道厚真町",
	158400: "北海道洞爺湖町",
	158500: "北海道安平町",
	158600: "北海道むかわ町",
	160100: "北海道日高町",
	160200: "北海道平取町",
	160400: "北海道新冠町",
	160700: "北海道浦河町",
	160800: "北海道様似町",
	160900: "北海道えりも町",
	161000: "北海道新ひだか町",
	163100: "北海道音更町",
	163200: "北海道士幌町",
	163300: "北海道上士幌町",
	163400: "北海道鹿追町",
	163500: "北海道新得町",
	163600: "北海道清水町",
	163700: "北海道芽室町",
	163800: "北海道中札内村",
	163900: "北海道更別村",
	164100: "北海道大樹町",
	164200: "北海道広尾町",
	164300: "北海道幕別町",
	164400: "北海道池田町",
	164500: "北海道豊頃町",
	164600: "北海道本別町",
	164700: "北海道足寄町",
	164800: "北海道陸別町",
	164900: "北海道浦幌町",
	166100: "北海道釧路町",
	166200: "北海道厚岸町",
	166300: "北海道浜中町",
	166400: "北海道標茶町",
	166500: "北海道弟子屈町",
	166700: "北海道鶴居村",
	166800: "北海道白糠町",
	169001: "北海道国後島",
	169002: "北海道択捉島",
	169100: "北海道別海町",
	169200: "北海道中標津町",
	169300: "北海道標津町",
	169400: "北海道羅臼町",
	169500: "北海道色丹村",
	199999: "北海道のその他の市町村",
	220100: "青森県青森市",
	220200: "青森県弘前市",
	220300: "青森県八戸市",
	220400: "青森県黒石市",
	220500: "青森県五所川原市",
	220600: "青森県十和田市",
	220700: "青森県三沢市",
	220800: "青森県むつ市",
	220900: "青森県つがる市",
	221000: "青森県平川市",
	230100: "青森県平内町",
	230300: "青森県今別町",
	230400: "青森県蓬田村",
	230700: "青森県外ヶ浜町",
	232100: "青森県鰺ヶ沢町",
	232300: "青森県深浦町",
	234300: "青森県西目屋村",
	236100: "青森県藤崎町",
	236200: "青森県大鰐町",
	236700: "青森県田舎館村",
	238100: "青森県板柳町",
	238400: "青森県鶴田町",
	238700: "青森県中泊町",
	240100: "青森県野辺地町",
	240200: "青森県七戸町",
	240500: "青森県六戸町",
	240600: "青森県横浜町",
	240800: "青森県東北町",
	241100: "青森県六ヶ所村",
	241200: "青森県おいらせ町",
	242300: "青森県大間町",
	242400: "青森県東通村",
	242500: "青森県風間浦村",
	242600: "青森県佐井村",
	244100: "青森県三戸町",
	244200: "青森県五戸町",
	244300: "青森県田子町",
	244500: "青森県南部町",
	244600: "青森県階上町",
	245000: "青森県新郷村",
	299999: "青森県のその他の市町村",
	320100: "岩手県盛岡市",
	320200: "岩手県宮古市",
	320300: "岩手県大船渡市",
	320500: "岩手県花巻市",
	320600: "岩手県北上市",
	320700: "岩手県久慈市",
	320800: "岩手県遠野市",
	320900: "岩手県一関市",
	321000: "岩手県陸前高田市",
	321100: "岩手県釜石市",
	321300: "岩手県二戸市",
	321400: "岩手県八幡平市",
	321500: "岩手県奥州市",
	321600: "岩手県滝沢市",
	330100: "岩手県雫石町",
	330200: "岩手県葛巻町",
	330300: "岩手県岩手町",
	332100: "岩手県紫波町",
	332200: "岩手県矢巾町",
	336600: "岩手県西和賀町",
	338100: "岩手県金ケ崎町",
	340200: "岩手県平泉町",
	344100: "岩手県住田町",
	346100: "岩手県大槌町",
	348200: "岩手県山田町",
	348300: "岩手県岩泉町",
	348400: "岩手県田野畑村",
	348500: "岩手県普代村",
	350100: "岩手県軽米町",
	350300: "岩手県野田村",
	350600: "岩手県九戸村",
	350700: "岩手県洋野町",
	352400: "岩手県一戸町",
	399999: "岩手県のその他の市町村",
	410000: "宮城県仙台市",
	420200: "宮城県石巻市",
	420300: "宮城県塩竈市",
	420500: "宮城県気仙沼市",
	420600: "宮城県白石市",
	420700: "宮城県名取市",
	420800: "宮城県角田市",
	420900: "宮城県多賀城市",
	421100: "宮城県岩沼市",
	421200: "宮城県登米市",
	421300: "宮城県栗原市",
	421400: "宮城県東松島市",
	421500: "宮城県大崎市",
	421600: "宮城県富谷市",
	430100: "宮城県蔵王町",
	430200: "宮城県七ヶ宿町",
	432100: "宮城県大河原町",
	432200: "宮城県村田町",
	432300: "宮城県柴田町",
	432400: "宮城県川崎町",
	434100: "宮城県丸森町",
	436100: "宮城県亘理町",
	436200: "宮城県山元町",
	440100: "宮城県松島町",
	440400: "宮城県七ヶ浜町",
	440600: "宮城県利府町",
	442100: "宮城県大和町",
	442200: "宮城県大郷町",
	442400: "宮城県大衡村",
	444400: "宮城県色麻町",
	444500: "宮城県加美町",
	450100: "宮城県涌谷町",
	450500: "宮城県美里町",
	458100: "宮城県女川町",
	460600: "宮城県南三陸町",
	499999: "宮城県のその他の市町村",
	520100: "秋田県秋田市",
	520200: "秋田県能代市",
	520300: "秋田県横手市",
	520400: "秋田県大館市",
	520600: "秋田県男鹿市",
	520700: "秋田県湯沢市",
	520900: "秋田県鹿角市",
	521000: "秋田県由利本荘市",
	521100: "秋田県潟上市",
	521200: "秋田県大仙市",
	521300: "秋田県北秋田市",
	521400: "秋田県にかほ市",
	521500: "秋田県仙北市",
	530300: "秋田県小坂町",
	532700: "秋田県上小阿仁村",
	534600: "秋田県藤里町",
	534800: "秋田県三種町",
	534900: "秋田県八峰町",
	536100: "秋田県五城目町",
	536300: "秋田県八郎潟町",
	536600: "秋田県井川町",
	536800: "秋田県大潟村",
	543400: "秋田県美郷町",
	546300: "秋田県羽後町",
	546400: "秋田県東成瀬村",
	599999: "秋田県のその他の市町村",
	620100: "山形県山形市",
	620200: "山形県米沢市",
	620300: "山形県鶴岡市",
	620400: "山形県酒田市",
	620500: "山形県新庄市",
	620600: "山形県寒河江市",
	620700: "山形県上山市",
	620800: "山形県村山市",
	620900: "山形県長井市",
	621000: "山形県天童市",
	621100: "山形県東根市",
	621200: "山形県尾花沢市",
	621300: "山形県南陽市",
	630100: "山形県山辺町",
	630200: "山形県中山町",
	632100: "山形県河北町",
	632200: "山形県西川町",
	632300: "山形県朝日町",
	632400: "山形県大江町",
	634100: "山形県大石田町",
	636100: "山形県金山町",
	636200: "山形県最上町",
	636300: "山形県舟形町",
	636400: "山形県真室川町",
	636500: "山形県大蔵村",
	636600: "山形県鮭川村",
	636700: "山形県戸沢村",
	638100: "山形県高畠町",
	638200: "山形県川西町",
	640100: "山形県小国町",
	640200: "山形県白鷹町",
	640300: "山形県飯豊町",
	642600: "山形県三川町",
	642800: "山形県庄内町",
	646100: "山形県遊佐町",
	699999: "山形県のその他の市町村",
	720100: "福島県福島市",
	720200: "福島県会津若松市",
	720300: "福島県郡山市",
	720400: "福島県いわき市",
	720500: "福島県白河市",
	720700: "福島県須賀川市",
	720800: "福島県喜多方市",
	720900: "福島県相馬市",
	721000: "福島県二本松市",
	721100: "福島県田村市",
	721200: "福島県南相馬市",
	721300: "福島県伊達市",
	721400: "福島県本宮市",
	730100: "福島県桑折町",
	730300: "福島県国見町",
	730800: "福島県川俣町",
	732200: "福島県大玉村",
	734200: "福島県鏡石町",
	734400: "福島県天栄村",
	736200: "福島県下郷町",
	736400: "福島県檜枝岐村",
	736700: "福島県只見町",
	736800: "福島県南会津町",
	740200: "福島県北塩原村",
	740500: "福島県西会津町",
	740700: "福島県磐梯町",
	740800: "福島県猪苗代町",
	742100: "福島県会津坂下町",
	742200: "福島県湯川村",
	742300: "福島県柳津町",
	744400: "福島県三島町",
	744500: "福島県金山町",
	744600: "福島県昭和村",
	744700: "福島県会津美里町",
	746100: "福島県西郷村",
	746400: "福島県泉崎村",
	746500: "福島県中島村",
	746600: "福島県矢吹町",
	748100: "福島県棚倉町",
	748200: "福島県矢祭町",
	748300: "福島県塙町",
	748400: "福島県鮫川村",
	750100: "福島県石川町",
	750200: "福島県玉川村",
	750300: "福島県平田村",
	750400: "福島県浅川町",
	750500: "福島県古殿町",
	752100: "福島県三春町",
	752200: "福島県小野町",
	754100: "福島県広野町",
	754200: "福島県楢葉町",
	754300: "福島県富岡町",
	754400: "福島県川内村",
	754500: "福島県大熊町",
	754600: "福島県双葉町",
	754700: "福島県浪江町",
	754800: "福島県葛尾村",
	756100: "福島県新地町",
	756400: "福島県飯舘村",
	799999: "福島県のその他の市町村",
	820100: "茨城県水戸市",
	820200: "茨城県日立市",
	820300: "茨城県土浦市",
	820400: "茨城県古河市",
	820500: "茨城県石岡市",
	820700: "茨城県結城市",
	820800: "茨城県龍ケ崎市",
	821000: "茨城県下妻市",
	821100: "茨城県常総市",
	821200: "茨城県常陸太田市",
	821400: "茨城県高萩市",
	821500: "茨城県北茨城市",
	821600: "茨城県笠間市",
	821700: "茨城県取手市",
	821900: "茨城県牛久市",
	822000: "茨城県つくば市",
	822100: "茨城県ひたちなか市",
	822200: "茨城県鹿嶋市",
	822300: "茨城県潮来市",
	822400: "茨城県守谷市",
	822500: "茨城県常陸大宮市",
	822600: "茨城県那珂市",
	822700: "茨城県筑西市",
	822800: "茨城県坂東市",
	822900: "茨城県稲敷市",
	823000: "茨城県かすみがうら市",
	823100: "茨城県桜川市",
	823200: "茨城県神栖市",
	823300: "茨城県行方市",
	823400: "茨城県鉾田市",
	823500: "茨城県つくばみらい市",
	823600: "茨城県小美玉市",
	830200: "茨城県茨城町",
	830900: "茨城県大洗町",
	831000: "茨城県城里町",
	834100: "茨城県東海村",
	836400: "茨城県大子町",
	844200: "茨城県美浦村",
	844300: "茨城県阿見町",
	844700: "茨城県河内町",
	852100: "茨城県八千代町",
	854200: "茨城県五霞町",
	854600: "茨城県境町",
	856400: "茨城県利根町",
	899999: "茨城県のその他の市町村",
	920100: "栃木県宇都宮市",
	920200: "栃木県足利市",
	920300: "栃木県栃木市",
	920400: "栃木県佐野市",
	920500: "栃木県鹿沼市",
	920600: "栃木県日光市",
	920800: "栃木県小山市",
	920900: "栃木県真岡市",
	921000: "栃木県大田原市",
	921100: "栃木県矢板市",
	921300: "栃木県那須塩原市",
	921400: "栃木県さくら市",
	921500: "栃木県那須烏山市",
	921600: "栃木県下野市",
	930100: "栃木県上三川町",
	934200: "栃木県益子町",
	934300: "栃木県茂木町",
	934400: "栃木県市貝町",
	934500: "栃木県芳賀町",
	936100: "栃木県壬生町",
	936400: "栃木県野木町",
	938400: "栃木県塩谷町",
	938600: "栃木県高根沢町",
	940700: "栃木県那須町",
	941100: "栃木県那珂川町",
	999999: "栃木県のその他の市町村",
	1020100: "群馬県前橋市",
	1020200: "群馬県高崎市",
	1020300: "群馬県桐生市",
	1020400: "群馬県伊勢崎市",
	1020500: "群馬県太田市",
	1020600: "群馬県沼田市",
	1020700: "群馬県館林市",
	1020800: "群馬県渋川市",
	1020900: "群馬県藤岡市",
	1021000: "群馬県富岡市",
	1021100: "群馬県安中市",
	1021200: "群馬県みどり市",
	1034400: "群馬県榛東村",
	1034500: "群馬県吉岡町",
	1036600: "群馬県上野村",
	1036700: "群馬県神流町",
	1038200: "群馬県下仁田町",
	1038300: "群馬県南牧村",
	1038400: "群馬県甘楽町",
	1042100: "群馬県中之条町",
	1042400: "群馬県長野原町",
	1042500: "群馬県嬬恋村",
	1042600: "群馬県草津町",
	1042800: "群馬県高山村",
	1042900: "群馬県東吾妻町",
	1044300: "群馬県片品村",
	1044400: "群馬県川場村",
	1044800: "群馬県昭和村",
	1044900: "群馬県みなかみ町",
	1046400: "群馬県玉村町",
	1052100: "群馬県板倉町",
	1052200: "群馬県明和町",
	1052300: "群馬県千代田町",
	1052400: "群馬県大泉町",
	1052500: "群馬県邑楽町",
	1099999: "群馬県のその他の市町村",
	1110000: "埼玉県さいたま市",
	1120100: "埼玉県川越市",
	1120200: "埼玉県熊谷市",
	1120300: "埼玉県川口市",
	1120600: "埼玉県行田市",
	1120700: "埼玉県秩父市",
	1120800: "埼玉県所沢市",
	1120900: "埼玉県飯能市",
	1121000: "埼玉県加須市",
	1121100: "埼玉県本庄市",
	1121200: "埼玉県東松山市",
	1121400: "埼玉県春日部市",
	1121500: "埼玉県狭山市",
	1121600: "埼玉県羽生市",
	1121700: "埼玉県鴻巣市",
	1121800: "埼玉県深谷市",
	1121900: "埼玉県上尾市",
	1122100: "埼玉県草加市",
	1122200: "埼玉県越谷市",
	1122300: "埼玉県蕨市",
	1122400: "埼玉県戸田市",
	1122500: "埼玉県入間市",
	1122700: "埼玉県朝霞市",
	1122800: "埼玉県志木市",
	1122900: "埼玉県和光市",
	1123000: "埼玉県新座市",
	1123100: "埼玉県桶川市",
	1123200: "埼玉県久喜市",
	1123300: "埼玉県北本市",
	1123400: "埼玉県八潮市",
	1123500: "埼玉県富士見市",
	1123700: "埼玉県三郷市",
	1123800: "埼玉県蓮田市",
	1123900: "埼玉県坂戸市",
	1124000: "埼玉県幸手市",
	1124100: "埼玉県鶴ヶ島市",
	1124200: "埼玉県日高市",
	1124300: "埼玉県吉川市",
	1124500: "埼玉県ふじみ野市",
	1124600: "埼玉県白岡市",
	1130100: "埼玉県伊奈町",
	1132400: "埼玉県三芳町",
	1132600: "埼玉県毛呂山町",
	1132700: "埼玉県越生町",
	1134100: "埼玉県滑川町",
	1134200: "埼玉県嵐山町",
	1134300: "埼玉県小川町",
	1134600: "埼玉県川島町",
	1134700: "埼玉県吉見町",
	1134800: "埼玉県鳩山町",
	1134900: "埼玉県ときがわ町",
	1136100: "埼玉県横瀬町",
	1136200: "埼玉県皆野町",
	1136300: "埼玉県長瀞町",
	1136500: "埼玉県小鹿野町",
	1136900: "埼玉県東秩父村",
	1138100: "埼玉県美里町",
	1138300: "埼玉県神川町",
	1138500: "埼玉県上里町",
	1140800: "埼玉県寄居町",
	1144200: "埼玉県宮代町",
	1146400: "埼玉県杉戸町",
	1146500: "埼玉県松伏町",
	1199999: "埼玉県のその他の市町村",
	1210000: "千葉県千葉市",
	1220200: "千葉県銚子市",
	1220300: "千葉県市川市",
	1220400: "千葉県船橋市",
	1220500: "千葉県館山市",
	1220600: "千葉県木更津市",
	1220700: "千葉県松戸市",
	1220800: "千葉県野田市",
	1221000: "千葉県茂原市",
	1221100: "千葉県成田市",
	1221200: "千葉県佐倉市",
	1221300: "千葉県東金市",
	1221500: "千葉県旭市",
	1221600: "千葉県習志野市",
	1221700: "千葉県柏市",
	1221800: "千葉県勝浦市",
	1221900: "千葉県市原市",
	1222000: "千葉県流山市",
	1222100: "千葉県八千代市",
	1222200: "千葉県我孫子市",
	1222300: "千葉県鴨川市",
	1222400: "千葉県鎌ケ谷市",
	1222500: "千葉県君津市",
	1222600: "千葉県富津市",
	1222700: "千葉県浦安市",
	1222800: "千葉県四街道市",
	1222900: "千葉県袖ケ浦市",
	1223000: "千葉県八街市",
	1223100: "千葉県印西市",
	1223200: "千葉県白井市",
	1223300: "千葉県富里市",
	1223400: "千葉県南房総市",
	1223500: "千葉県匝瑳市",
	1223600: "千葉県香取市",
	1223700: "千葉県山武市",
	1223800: "千葉県いすみ市",
	1223900: "千葉県大網白里市",
	1232200: "千葉県酒々井町",
	1232900: "千葉県栄町",
	1234200: "千葉県神崎町",
	1234700: "千葉県多古町",
	1234900: "千葉県東庄町",
	1240300: "千葉県九十九里町",
	1240900: "千葉県芝山町",
	1241000: "千葉県横芝光町",
	1242100: "千葉県一宮町",
	1242200: "千葉県睦沢町",
	1242300: "千葉県長生村",
	1242400: "千葉県白子町",
	1242600: "千葉県長柄町",
	1242700: "千葉県長南町",
	1244100: "千葉県大多喜町",
	1244300: "千葉県御宿町",
	1246300: "千葉県鋸南町",
	1299999: "千葉県のその他の市町村",
	1310100: "東京都千代田区",
	1310200: "東京都中央区",
	1310300: "東京都港区",
	1310400: "東京都新宿区",
	1310500: "東京都文京区",
	1310600: "東京都台東区",
	1310700: "東京都墨田区",
	1310800: "東京都江東区",
	1310900: "東京都品川区",
	1311000: "東京都目黒区",
	1311100: "東京都大田区",
	1311200: "東京都世田谷区",
	1311300: "東京都渋谷区",
	1311400: "東京都中野区",
	1311500: "東京都杉並区",
	1311600: "東京都豊島区",
	1311700: "東京都北区",
	1311800: "東京都荒川区",
	1311900: "東京都板橋区",
	1312000: "東京都練馬区",
	1312100: "東京都足立区",
	1312200: "東京都葛飾区",
	1312300: "東京都江戸川区",
	1320100: "東京都八王子市",
	1320200: "東京都立川市",
	1320300: "東京都武蔵野市",
	1320400: "東京都三鷹市",
	1320500: "東京都青梅市",
	1320600: "東京都府中市",
	1320700: "東京都昭島市",
	1320800: "東京都調布市",
	1320900: "東京都町田市",
	1321000: "東京都小金井",
	1321100: "東京都小平市",
	1321200: "東京都日野市",
	1321300: "東京都東村山市",
	1321400: "東京都国分寺市",
	1321500: "東京都国立市",
	1321800: "東京都福生市",
	1321900: "東京都狛江市",
	1322000: "東京都東大和市",
	1322100: "東京都清瀬市",
	1322200: "東京都東久留米市",
	1322300: "東京都武蔵村山市",
	1322400: "東京都多摩市",
	1322500: "東京都稲城市",
	1322700: "東京都羽村市",
	1322800: "東京都あきる野市",
	1322900: "東京都西東京市",
	1330300: "東京都瑞穂町",
	1330500: "東京都日の出町",
	1330700: "東京都檜原村",
	1330800: "東京都奥多摩町",
	1336100: "東京都大島町",
	1336200: "東京都利島村",
	1336300: "東京都新島村",
	1336400: "東京都神津島村",
	1338100: "東京都三宅村",
	1338200: "東京都御蔵島村",
	1340000: "東京都八丈支庁",
	1340100: "東京都八丈町",
	1340200: "東京都青ヶ島村",
	1342100: "東京都小笠原村",
	1399999: "東京都のその他の市町村",
	1410000: "神奈川県横浜市",
	1413000: "神奈川県川崎市",
	1415000: "神奈川県相模原市",
	1420100: "神奈川県横須賀市",
	1420300: "神奈川県平塚市",
	1420400: "神奈川県鎌倉市",
	1420500: "神奈川県藤沢市",
	1420600: "神奈川県小田原市",
	1420700: "神奈川県茅ヶ崎市",
	1420800: "神奈川県逗子市",
	1421000: "神奈川県三浦市",
	1421100: "神奈川県秦野市",
	1421200: "神奈川県厚木市",
	1421300: "神奈川県大和市",
	1421400: "神奈川県伊勢原市",
	1421500: "神奈川県海老名市",
	1421600: "神奈川県座間市",
	1421700: "神奈川県南足柄市",
	1421800: "神奈川県綾瀬市",
	1430100: "神奈川県葉山町",
	1432100: "神奈川県寒川町",
	1434100: "神奈川県大磯町",
	1434200: "神奈川県二宮町",
	1436100: "神奈川県中井町",
	1436200: "神奈川県大井町",
	1436300: "神奈川県松田町",
	1436400: "神奈川県山北町",
	1436600: "神奈川県開成町",
	1438200: "神奈川県箱根町",
	1438300: "神奈川県真鶴町",
	1438400: "神奈川県湯河原町",
	1440100: "神奈川県愛川町",
	1440200: "神奈川県清川村",
	1499999: "神奈川県のその他の市町村",
	1510000: "新潟県新潟市",
	1520200: "新潟県長岡市",
	1520400: "新潟県三条市",
	1520500: "新潟県柏崎市",
	1520600: "新潟県新発田市",
	1520800: "新潟県小千谷市",
	1520900: "新潟県加茂市",
	1521000: "新潟県十日町市",
	1521100: "新潟県見附市",
	1521200: "新潟県村上市",
	1521300: "新潟県燕市",
	1521600: "新潟県糸魚川市",
	1521700: "新潟県妙高市",
	1521800: "新潟県五泉市",
	1522200: "新潟県上越市",
	1522300: "新潟県阿賀野市",
	1522400: "新潟県佐渡市",
	1522500: "新潟県魚沼市",
	1522600: "新潟県南魚沼市",
	1522700: "新潟県胎内市",
	1530700: "新潟県聖籠町",
	1534200: "新潟県弥彦村",
	1536100: "新潟県田上町",
	1538500: "新潟県阿賀町",
	1540500: "新潟県出雲崎町",
	1546100: "新潟県湯沢町",
	1548200: "新潟県津南町",
	1550400: "新潟県刈羽村",
	1558100: "新潟県関川村",
	1558600: "新潟県粟島浦村",
	1599999: "新潟県のその他の市町村",
	1620100: "富山県富山市",
	1620200: "富山県高岡市",
	1620400: "富山県魚津市",
	1620500: "富山県氷見市",
	1620600: "富山県滑川市",
	1620700: "富山県黒部市",
	1620800: "富山県砺波市",
	1620900: "富山県小矢部市",
	1621000: "富山県南砺市",
	1621100: "富山県射水市",
	1632100: "富山県舟橋村",
	1632200: "富山県上市町",
	1632300: "富山県立山町",
	1634200: "富山県入善町",
	1634300: "富山県朝日町",
	1699999: "富山県のその他の市町村",
	1720100: "石川県金沢市",
	1720200: "石川県七尾市",
	1720300: "石川県小松市",
	1720400: "石川県輪島市",
	1720500: "石川県珠洲市",
	1720600: "石川県加賀市",
	1720700: "石川県羽咋市",
	1720900: "石川県かほく市",
	1721000: "石川県白山市",
	1721100: "石川県能美市",
	1721200: "石川県野々市市",
	1732400: "石川県川北町",
	1736100: "石川県津幡町",
	1736500: "石川県内灘町",
	1738400: "石川県志賀町",
	1738600: "石川県宝達志水町",
	1740700: "石川県中能登町",
	1746100: "石川県穴水町",
	1746300: "石川県能登町",
	1799999: "石川県のその他の市町村",
	1820100: "福井県福井市",
	1820200: "福井県敦賀市",
	1820400: "福井県小浜市",
	1820500: "福井県大野市",
	1820600: "福井県勝山市",
	1820700: "福井県鯖江市",
	1820800: "福井県あわら市",
	1820900: "福井県越前市",
	1821000: "福井県坂井市",
	1832200: "福井県永平寺町",
	1838200: "福井県池田町",
	1840400: "福井県南越前町",
	1842300: "福井県越前町",
	1844200: "福井県美浜町",
	1848100: "福井県高浜町",
	1848300: "福井県おおい町",
	1850100: "福井県若狭町",
	1899999: "福井県のその他の市町村",
	1920100: "山梨県甲府市",
	1920200: "山梨県富士吉田市",
	1920400: "山梨県都留市",
	1920500: "山梨県山梨市",
	1920600: "山梨県大月市",
	1920700: "山梨県韮崎市",
	1920800: "山梨県南アルプス市",
	1920900: "山梨県北杜市",
	1921000: "山梨県甲斐市",
	1921100: "山梨県笛吹市",
	1921200: "山梨県上野原市",
	1921300: "山梨県甲州市",
	1921400: "山梨県中央市",
	1934600: "山梨県市川三郷町",
	1936400: "山梨県早川町",
	1936500: "山梨県身延町",
	1936600: "山梨県南部町",
	1936800: "山梨県富士川町",
	1938400: "山梨県昭和町",
	1942200: "山梨県道志村",
	1942300: "山梨県西桂町",
	1942400: "山梨県忍野村",
	1942500: "山梨県山中湖村",
	1942900: "山梨県鳴沢村",
	1943000: "山梨県富士河口湖町",
	1944200: "山梨県小菅村",
	1944300: "山梨県丹波山村",
	1999999: "山梨県のその他の市町村",
	2020100: "長野県長野市",
	2020200: "長野県松本市",
	2020300: "長野県上田市",
	2020400: "長野県岡谷市",
	2020500: "長野県飯田市",
	2020600: "長野県諏訪市",
	2020700: "長野県須坂市",
	2020800: "長野県小諸市",
	2020900: "長野県伊那市",
	2021000: "長野県駒ヶ根市",
	2021100: "長野県中野市",
	2021200: "長野県大町市",
	2021300: "長野県飯山市",
	2021400: "長野県茅野市",
	2021500: "長野県塩尻市",
	2021700: "長野県佐久市",
	2021800: "長野県千曲市",
	2021900: "長野県東御市",
	2022000: "長野県安曇野市",
	2030300: "長野県小海町",
	2030400: "長野県川上村",
	2030500: "長野県南牧村",
	2030600: "長野県南相木村",
	2030700: "長野県北相木村",
	2030900: "長野県佐久穂町",
	2032100: "長野県軽井沢町",
	2032300: "長野県御代田町",
	2032400: "長野県立科町",
	2034900: "長野県青木村",
	2035000: "長野県長和町",
	2036100: "長野県下諏訪町",
	2036200: "長野県富士見町",
	2036300: "長野県原村",
	2038200: "長野県辰野町",
	2038300: "長野県箕輪町",
	2038400: "長野県飯島町",
	2038500: "長野県南箕輪村",
	2038600: "長野県中川村",
	2038800: "長野県宮田村",
	2040200: "長野県松川町",
	2040300: "長野県高森町",
	2040400: "長野県阿南町",
	2040700: "長野県阿智村",
	2040900: "長野県平谷村",
	2041000: "長野県根羽村",
	2041100: "長野県下條村",
	2041200: "長野県売木村",
	2041300: "長野県天龍村",
	2041400: "長野県泰阜村",
	2041500: "長野県喬木村",
	2041600: "長野県豊丘村",
	2041700: "長野県大鹿村",
	2042200: "長野県上松町",
	2042300: "長野県南木曽町",
	2042500: "長野県木祖村",
	2042900: "長野県王滝村",
	2043000: "長野県大桑村",
	2043200: "長野県木曽町",
	2044600: "長野県麻績村",
	2044800: "長野県生坂村",
	2045000: "長野県山形村",
	2045100: "長野県朝日村",
	2045200: "長野県筑北村",
	2048100: "長野県池田町",
	2048200: "長野県松川村",
	2048500: "長野県白馬村",
	2048600: "長野県小谷村",
	2052100: "長野県坂城町",
	2054100: "長野県小布施町",
	2054300: "長野県高山村",
	2056100: "長野県山ノ内町",
	2056200: "長野県木島平村",
	2056300: "長野県野沢温泉村",
	2058300: "長野県信濃町",
	2058800: "長野県小川村",
	2059000: "長野県飯綱町",
	2060200: "長野県栄村",
	2099999: "長野県のその他の市町村",
	2120100: "岐阜県岐阜市",
	2120200: "岐阜県大垣市",
	2120300: "岐阜県高山市",
	2120400: "岐阜県多治見市",
	2120500: "岐阜県関市",
	2120600: "岐阜県中津川市",
	2120700: "岐阜県美濃市",
	2120800: "岐阜県瑞浪市",
	2120900: "岐阜県羽島市",
	2121000: "岐阜県恵那市",
	2121100: "岐阜県美濃加茂市",
	2121200: "岐阜県土岐市",
	2121300: "岐阜県各務原市",
	2121400: "岐阜県可児市",
	2121500: "岐阜県山県市",
	2121600: "岐阜県瑞穂市",
	2121700: "岐阜県飛騨市",
	2121800: "岐阜県本巣市",
	2121900: "岐阜県郡上市",
	2122000: "岐阜県下呂市",
	2122100: "岐阜県海津市",
	2130200: "岐阜県岐南町",
	2130300: "岐阜県笠松町",
	2134100: "岐阜県養老町",
	2136100: "岐阜県垂井町",
	2136200: "岐阜県関ケ原町",
	2138100: "岐阜県神戸町",
	2138200: "岐阜県輪之内町",
	2138300: "岐阜県安八町",
	2140100: "岐阜県揖斐川町",
	2140300: "岐阜県大野町",
	2140400: "岐阜県池田町",
	2142100: "岐阜県北方町",
	2150100: "岐阜県坂祝町",
	2150200: "岐阜県富加町",
	2150300: "岐阜県川辺町",
	2150400: "岐阜県七宗町",
	2150500: "岐阜県八百津町",
	2150600: "岐阜県白川町",
	2150700: "岐阜県東白川村",
	2152100: "岐阜県御嵩町",
	2160400: "岐阜県白川村",
	2199999: "岐阜県のその他の市町村",
	2210000: "静岡県静岡市",
	2213000: "静岡県浜松市",
	2220300: "静岡県沼津市",
	2220500: "静岡県熱海市",
	2220600: "静岡県三島市",
	2220700: "静岡県富士宮市",
	2220800: "静岡県伊東市",
	2220900: "静岡県島田市",
	2221000: "静岡県富士市",
	2221100: "静岡県磐田市",
	2221200: "静岡県焼津市",
	2221300: "静岡県掛川市",
	2221400: "静岡県藤枝市",
	2221500: "静岡県御殿場市",
	2221600: "静岡県袋井市",
	2221900: "静岡県下田市",
	2222000: "静岡県裾野市",
	2222100: "静岡県湖西市",
	2222200: "静岡県伊豆市",
	2222300: "静岡県御前崎市",
	2222400: "静岡県菊川市",
	2222500: "静岡県伊豆の国市",
	2222600: "静岡県牧之原市",
	2230100: "静岡県東伊豆町",
	2230200: "静岡県河津町",
	2230400: "静岡県南伊豆町",
	2230500: "静岡県松崎町",
	2230600: "静岡県西伊豆町",
	2232500: "静岡県函南町",
	2234100: "静岡県清水町",
	2234200: "静岡県長泉町",
	2234400: "静岡県小山町",
	2242400: "静岡県吉田町",
	2242900: "静岡県川根本町",
	2246100: "静岡県森町",
	2299999: "静岡県のその他の市町村",
	2310000: "愛知県名古屋市",
	2320100: "愛知県豊橋市",
	2320200: "愛知県岡崎市",
	2320300: "愛知県一宮市",
	2320400: "愛知県瀬戸市",
	2320500: "愛知県半田市",
	2320600: "愛知県春日井市",
	2320700: "愛知県豊川市",
	2320800: "愛知県津島市",
	2320900: "愛知県碧南市",
	2321000: "愛知県刈谷市",
	2321100: "愛知県豊田市",
	2321200: "愛知県安城市",
	2321300: "愛知県西尾市",
	2321400: "愛知県蒲郡市",
	2321500: "愛知県犬山市",
	2321600: "愛知県常滑市",
	2321700: "愛知県江南市",
	2321900: "愛知県小牧市",
	2322000: "愛知県稲沢市",
	2322100: "愛知県新城市",
	2322200: "愛知県東海市",
	2322300: "愛知県大府市",
	2322400: "愛知県知多市",
	2322500: "愛知県知立市",
	2322600: "愛知県尾張旭市",
	2322700: "愛知県高浜市",
	2322800: "愛知県岩倉市",
	2322900: "愛知県豊明市",
	2323000: "愛知県日進市",
	2323100: "愛知県田原市",
	2323200: "愛知県愛西市",
	2323300: "愛知県清須市",
	2323400: "愛知県北名古屋市",
	2323500: "愛知県弥富市",
	2323600: "愛知県みよし市",
	2323700: "愛知県あま市",
	2323800: "愛知県長久手市",
	2330200: "愛知県東郷町",
	2334200: "愛知県豊山町",
	2336100: "愛知県大口町",
	2336200: "愛知県扶桑町",
	2342400: "愛知県大治町",
	2342500: "愛知県蟹江町",
	2342700: "愛知県飛島村",
	2344100: "愛知県阿久比町",
	2344200: "愛知県東浦町",
	2344500: "愛知県南知多町",
	2344600: "愛知県美浜町",
	2344700: "愛知県武豊町",
	2350100: "愛知県幸田町",
	2356100: "愛知県設楽町",
	2356200: "愛知県東栄町",
	2356300: "愛知県豊根村",
	2399999: "愛知県のその他の市町村",
	2420100: "三重県津市",
	2420200: "三重県四日市市",
	2420300: "三重県伊勢市",
	2420400: "三重県松阪市",
	2420500: "三重県桑名市",
	2420700: "三重県鈴鹿市",
	2420800: "三重県名張市",
	2420900: "三重県尾鷲市",
	2421000: "三重県亀山市",
	2421100: "三重県鳥羽市",
	2421200: "三重県熊野市",
	2421400: "三重県いなべ市",
	2421500: "三重県志摩市",
	2421600: "三重県伊賀市",
	2430300: "三重県木曽岬町",
	2432400: "三重県東員町",
	2434100: "三重県菰野町",
	2434300: "三重県朝日町",
	2434400: "三重県川越町",
	2444100: "三重県多気町",
	2444200: "三重県明和町",
	2444300: "三重県大台町",
	2446100: "三重県玉城町",
	2447000: "三重県度会町",
	2447100: "三重県大紀町",
	2447200: "三重県南伊勢町",
	2454300: "三重県紀北町",
	2456100: "三重県御浜町",
	2456200: "三重県紀宝町",
	2499999: "三重県のその他の市町村",
	2520100: "滋賀県大津市",
	2520200: "滋賀県彦根市",
	2520300: "滋賀県長浜市",
	2520400: "滋賀県近江八幡市",
	2520600: "滋賀県草津市",
	2520700: "滋賀県守山市",
	2520800: "滋賀県栗東市",
	2520900: "滋賀県甲賀市",
	2521000: "滋賀県野洲市",
	2521100: "滋賀県湖南市",
	2521200: "滋賀県高島市",
	2521300: "滋賀県東近江市",
	2521400: "滋賀県米原市",
	2538300: "滋賀県日野町",
	2538400: "滋賀県竜王町",
	2542500: "滋賀県愛荘町",
	2544100: "滋賀県豊郷町",
	2544200: "滋賀県甲良町",
	2544300: "滋賀県多賀町",
	2599999: "滋賀県のその他の市町村",
	2610000: "京都府京都市",
	2620100: "京都府福知山市",
	2620200: "京都府舞鶴市",
	2620300: "京都府綾部市",
	2620400: "京都府宇治市",
	2620500: "京都府宮津市",
	2620600: "京都府亀岡市",
	2620700: "京都府城陽市",
	2620800: "京都府向日市",
	2620900: "京都府長岡京市",
	2621000: "京都府八幡市",
	2621100: "京都府京田辺市",
	2621200: "京都府京丹後市",
	2621300: "京都府南丹市",
	2621400: "京都府木津川市",
	2630300: "京都府大山崎町",
	2632200: "京都府久御山町",
	2634300: "京都府井手町",
	2634400: "京都府宇治田原町",
	2636400: "京都府笠置町",
	2636500: "京都府和束町",
	2636600: "京都府精華町",
	2636700: "京都府南山城村",
	2640700: "京都府京丹波町",
	2646300: "京都府伊根町",
	2646500: "京都府与謝野町",
	2699999: "京都府のその他の市町村",
	2710000: "大阪府大阪市",
	2714000: "大阪府堺市",
	2720200: "大阪府岸和田市",
	2720300: "大阪府豊中市",
	2720400: "大阪府池田市",
	2720500: "大阪府吹田市",
	2720600: "大阪府泉大津市",
	2720700: "大阪府高槻市",
	2720800: "大阪府貝塚市",
	2720900: "大阪府守口市",
	2721000: "大阪府枚方市",
	2721100: "大阪府茨木市",
	2721200: "大阪府八尾市",
	2721300: "大阪府泉佐野市",
	2721400: "大阪府富田林市",
	2721500: "大阪府寝屋川市",
	2721600: "大阪府河内長野市",
	2721700: "大阪府松原市",
	2721800: "大阪府大東市",
	2721900: "大阪府和泉市",
	2722000: "大阪府箕面市",
	2722100: "大阪府柏原市",
	2722200: "大阪府羽曳野市",
	2722300: "大阪府門真市",
	2722400: "大阪府摂津市",
	2722500: "大阪府高石市",
	2722600: "大阪府藤井寺市",
	2722700: "大阪府東大阪市",
	2722800: "大阪府泉南市",
	2722900: "大阪府四條畷市",
	2723000: "大阪府交野市",
	2723100: "大阪府大阪狭山市",
	2723200: "大阪府阪南市",
	2730100: "大阪府島本町",
	2732100: "大阪府豊能町",
	2732200: "大阪府能勢町",
	2734100: "大阪府忠岡町",
	2736100: "大阪府熊取町",
	2736200: "大阪府田尻町",
	2736600: "大阪府岬町",
	2738100: "大阪府太子町",
	2738200: "大阪府河南町",
	2738300: "大阪府千早赤阪村",
	2799999: "大阪府のその他の市町村",
	2810000: "兵庫県神戸市",
	2820100: "兵庫県姫路市",
	2820200: "兵庫県尼崎市",
	2820300: "兵庫県明石市",
	2820400: "兵庫県西宮市",
	2820500: "兵庫県洲本市",
	2820600: "兵庫県芦屋市",
	2820700: "兵庫県伊丹市",
	2820800: "兵庫県相生市",
	2820900: "兵庫県豊岡市",
	2821000: "兵庫県加古川市",
	2821200: "兵庫県赤穂市",
	2821300: "兵庫県西脇市",
	2821400: "兵庫県宝塚市",
	2821500: "兵庫県三木市",
	2821600: "兵庫県高砂市",
	2821700: "兵庫県川西市",
	2821800: "兵庫県小野市",
	2821900: "兵庫県三田市",
	2822000: "兵庫県加西市",
	2822100: "兵庫県丹波篠山市",
	2822200: "兵庫県養父市",
	2822300: "兵庫県丹波市",
	2822400: "兵庫県南あわじ市",
	2822500: "兵庫県朝来市",
	2822600: "兵庫県淡路市",
	2822700: "兵庫県宍粟市",
	2822800: "兵庫県加東市",
	2822900: "兵庫県たつの市",
	2830100: "兵庫県猪名川町",
	2836500: "兵庫県多可町",
	2838100: "兵庫県稲美町",
	2838200: "兵庫県播磨町",
	2844200: "兵庫県市川町",
	2844300: "兵庫県福崎町",
	2844600: "兵庫県神河町",
	2846400: "兵庫県太子町",
	2848100: "兵庫県上郡町",
	2850100: "兵庫県佐用町",
	2858500: "兵庫県香美町",
	2858600: "兵庫県新温泉町",
	2899999: "兵庫県のその他の市町村",
	2920100: "奈良県奈良市",
	2920200: "奈良県大和高田市",
	2920300: "奈良県大和郡山市",
	2920400: "奈良県天理市",
	2920500: "奈良県橿原市",
	2920600: "奈良県桜井市",
	2920700: "奈良県五條市",
	2920800: "奈良県御所市",
	2920900: "奈良県生駒市",
	2921000: "奈良県香芝市",
	2921100: "奈良県葛城市",
	2921200: "奈良県宇陀市",
	2932200: "奈良県山添村",
	2934200: "奈良県平群町",
	2934300: "奈良県三郷町",
	2934400: "奈良県斑鳩町",
	2934500: "奈良県安堵町",
	2936100: "奈良県川西町",
	2936200: "奈良県三宅町",
	2936300: "奈良県田原本町",
	2938500: "奈良県曽爾村",
	2938600: "奈良県御杖村",
	2940100: "奈良県高取町",
	2940200: "奈良県明日香村",
	2942400: "奈良県上牧町",
	2942500: "奈良県王寺町",
	2942600: "奈良県広陵町",
	2942700: "奈良県河合町",
	2944100: "奈良県吉野町",
	2944200: "奈良県大淀町",
	2944300: "奈良県下市町",
	2944400: "奈良県黒滝村",
	2944600: "奈良県天川村",
	2944700: "奈良県野迫川村",
	2944900: "奈良県十津川村",
	2945000: "奈良県下北山村",
	2945100: "奈良県上北山村",
	2945200: "奈良県川上村",
	2945300: "奈良県東吉野村",
	2999999: "奈良県のその他の市町村",
	3020100: "和歌山県和歌山市",
	3020200: "和歌山県海南市",
	3020300: "和歌山県橋本市",
	3020400: "和歌山県有田市",
	3020500: "和歌山県御坊市",
	3020600: "和歌山県田辺市",
	3020700: "和歌山県新宮市",
	3020800: "和歌山県紀の川市",
	3020900: "和歌山県岩出市",
	3030400: "和歌山県紀美野町",
	3034100: "和歌山県かつらぎ町",
	3034300: "和歌山県九度山町",
	3034400: "和歌山県高野町",
	3036100: "和歌山県湯浅町",
	3036200: "和歌山県広川町",
	3036600: "和歌山県有田川町",
	3038100: "和歌山県美浜町",
	3038200: "和歌山県日高町",
	3038300: "和歌山県由良町",
	3039000: "和歌山県印南町",
	3039100: "和歌山県みなべ町",
	3039200: "和歌山県日高川町",
	3040100: "和歌山県白浜町",
	3040400: "和歌山県上富田町",
	3040600: "和歌山県すさみ町",
	3042100: "和歌山県那智勝浦町",
	3042200: "和歌山県太地町",
	3042400: "和歌山県古座川町",
	3042700: "和歌山県北山村",
	3042800: "和歌山県串本町",
	3099999: "和歌山県のその他の市町村",
	3120100: "鳥取県鳥取市",
	3120200: "鳥取県米子市",
	3120300: "鳥取県倉吉市",
	3120400: "鳥取県境港市",
	3130200: "鳥取県岩美町",
	3132500: "鳥取県若桜町",
	3132800: "鳥取県智頭町",
	3132900: "鳥取県八頭町",
	3136400: "鳥取県三朝町",
	3137000: "鳥取県湯梨浜町",
	3137100: "鳥取県琴浦町",
	3137200: "鳥取県北栄町",
	3138400: "鳥取県日吉津村",
	3138600: "鳥取県大山町",
	3138900: "鳥取県南部町",
	3139000: "鳥取県伯耆町",
	3140100: "鳥取県日南町",
	3140200: "鳥取県日野町",
	3140300: "鳥取県江府町",
	3199999: "鳥取県のその他の市町村",
	3220100: "島根県松江市",
	3220200: "島根県浜田市",
	3220300: "島根県出雲市",
	3220400: "島根県益田市",
	3220500: "島根県大田市",
	3220600: "島根県安来市",
	3220700: "島根県江津市",
	3220900: "島根県雲南市",
	3234300: "島根県奥出雲町",
	3238600: "島根県飯南町",
	3244100: "島根県川本町",
	3244800: "島根県美郷町",
	3244900: "島根県邑南町",
	3250100: "島根県津和野町",
	3250500: "島根県吉賀町",
	3252500: "島根県海士町",
	3252600: "島根県西ノ島町",
	3252700: "島根県知夫村",
	3252800: "島根県隠岐の島町",
	3299999: "島根県のその他の市町村",
	3310000: "岡山県岡山市",
	3320200: "岡山県倉敷市",
	3320300: "岡山県津山市",
	3320400: "岡山県玉野市",
	3320500: "岡山県笠岡市",
	3320700: "岡山県井原市",
	3320800: "岡山県総社市",
	3320900: "岡山県高梁市",
	3321000: "岡山県新見市",
	3321100: "岡山県備前市",
	3321200: "岡山県瀬戸内市",
	3321300: "岡山県赤磐市",
	3321400: "岡山県真庭市",
	3321500: "岡山県美作市",
	3321600: "岡山県浅口市",
	3334600: "岡山県和気町",
	3342300: "岡山県早島町",
	3344500: "岡山県里庄町",
	3346100: "岡山県矢掛町",
	3358600: "岡山県新庄村",
	3360600: "岡山県鏡野町",
	3362200: "岡山県勝央町",
	3362300: "岡山県奈義町",
	3364300: "岡山県西粟倉村",
	3366300: "岡山県久米南町",
	3366600: "岡山県美咲町",
	3368100: "岡山県吉備中央町",
	3399999: "岡山県のその他の市町村",
	3410000: "広島県広島市",
	3420200: "広島県呉市",
	3420300: "広島県竹原市",
	3420400: "広島県三原市",
	3420500: "広島県尾道市",
	3420700: "広島県福山市",
	3420800: "広島県府中市",
	3420900: "広島県三次市",
	3421000: "広島県庄原市",
	3421100: "広島県大竹市",
	3421200: "広島県東広島市",
	3421300: "広島県廿日市市",
	3421400: "広島県安芸高田市",
	3421500: "広島県江田島市",
	3430200: "広島県府中町",
	3430400: "広島県海田町",
	3430700: "広島県熊野町",
	3430900: "広島県坂町",
	3436800: "広島県安芸太田町",
	3436900: "広島県北広島町",
	3443100: "広島県大崎上島町",
	3446200: "広島県世羅町",
	3454500: "広島県神石高原町",
	3499999: "広島県のその他の市町村",
	3520100: "山口県下関市",
	3520200: "山口県宇部市",
	3520300: "山口県山口市",
	3520400: "山口県萩市",
	3520600: "山口県防府市",
	3520700: "山口県下松市",
	3520800: "山口県岩国市",
	3521000: "山口県光市",
	3521100: "山口県長門市",
	3521200: "山口県柳井市",
	3521300: "山口県美祢市",
	3521500: "山口県周南市",
	3521600: "山口県山陽小野田市",
	3530500: "山口県周防大島町",
	3532100: "山口県和木町",
	3534100: "山口県上関町",
	3534300: "山口県田布施町",
	3534400: "山口県平生町",
	3550200: "山口県阿武町",
	3599999: "山口県のその他の市町村",
	3620100: "徳島県徳島市",
	3620200: "徳島県鳴門市",
	3620300: "徳島県小松島市",
	3620400: "徳島県阿南市",
	3620500: "徳島県吉野川市",
	3620600: "徳島県阿波市",
	3620700: "徳島県美馬市",
	3620800: "徳島県三好市",
	3630100: "徳島県勝浦町",
	3630200: "徳島県上勝町",
	3632100: "徳島県佐那河内村",
	3634100: "徳島県石井町",
	3634200: "徳島県神山町",
	3636800: "徳島県那賀町",
	3638300: "徳島県牟岐町",
	3638700: "徳島県美波町",
	3638800: "徳島県海陽町",
	3640100: "徳島県松茂町",
	3640200: "徳島県北島町",
	3640300: "徳島県藍住町",
	3640400: "徳島県板野町",
	3640500: "徳島県上板町",
	3646800: "徳島県つるぎ町",
	3648900: "徳島県東みよし町",
	3699999: "徳島県のその他の市町村",
	3720100: "香川県高松市",
	3720200: "香川県丸亀市",
	3720300: "香川県坂出市",
	3720400: "香川県善通寺市",
	3720500: "香川県観音寺市",
	3720600: "香川県さぬき市",
	3720700: "香川県東かがわ市",
	3720800: "香川県三豊市",
	3732200: "香川県土庄町",
	3732400: "香川県小豆島町",
	3734100: "香川県三木町",
	3736400: "香川県直島町",
	3738600: "香川県宇多津町",
	3738700: "香川県綾川町",
	3740300: "香川県琴平町",
	3740400: "香川県多度津町",
	3740600: "香川県まんのう町",
	3799999: "香川県のその他の市町村",
	3820100: "愛媛県松山市",
	3820200: "愛媛県今治市",
	3820300: "愛媛県宇和島市",
	3820400: "愛媛県八幡浜市",
	3820500: "愛媛県新居浜市",
	3820600: "愛媛県西条市",
	3820700: "愛媛県大洲市",
	3821000: "愛媛県伊予市",
	3821300: "愛媛県四国中央市",
	3821400: "愛媛県西予市",
	3821500: "愛媛県東温市",
	3835600: "愛媛県上島町",
	3838600: "愛媛県久万高原町",
	3840100: "愛媛県松前町",
	3840200: "愛媛県砥部町",
	3842200: "愛媛県内子町",
	3844200: "愛媛県伊方町",
	3848400: "愛媛県松野町",
	3848800: "愛媛県鬼北町",
	3850600: "愛媛県愛南町",
	3899999: "愛媛県のその他の市町村",
	3920100: "高知県高知市",
	3920200: "高知県室戸市",
	3920300: "高知県安芸市",
	3920400: "高知県南国市",
	3920500: "高知県土佐市",
	3920600: "高知県須崎市",
	3920800: "高知県宿毛市",
	3920900: "高知県土佐清水市",
	3921000: "高知県四万十市",
	3921100: "高知県香南市",
	3921200: "高知県香美市",
	3930100: "高知県東洋町",
	3930200: "高知県奈半利町",
	3930300: "高知県田野町",
	3930400: "高知県安田町",
	3930500: "高知県北川村",
	3930600: "高知県馬路村",
	3930700: "高知県芸西村",
	3934100: "高知県本山町",
	3934400: "高知県大豊町",
	3936300: "高知県土佐町",
	3936400: "高知県大川村",
	3938600: "高知県いの町",
	3938700: "高知県仁淀川町",
	3940100: "高知県中土佐町",
	3940200: "高知県佐川町",
	3940300: "高知県越知町",
	3940500: "高知県檮原町",
	3941000: "高知県日高村",
	3941100: "高知県津野町",
	3941200: "高知県四万十町",
	3942400: "高知県大月町",
	3942700: "高知県三原村",
	3942800: "高知県黒潮町",
	3999999: "高知県のその他の市町村",
	4010000: "福岡県北九州市",
	4013000: "福岡県福岡市",
	4020200: "福岡県大牟田市",
	4020300: "福岡県久留米市",
	4020400: "福岡県直方市",
	4020500: "福岡県飯塚市",
	4020600: "福岡県田川市",
	4020700: "福岡県柳川市",
	4021000: "福岡県八女市",
	4021100: "福岡県筑後市",
	4021200: "福岡県大川市",
	4021300: "福岡県行橋市",
	4021400: "福岡県豊前市",
	4021500: "福岡県中間市",
	4021600: "福岡県小郡市",
	4021700: "福岡県筑紫野市",
	4021800: "福岡県春日市",
	4021900: "福岡県大野城市",
	4022000: "福岡県宗像市",
	4022100: "福岡県太宰府市",
	4022300: "福岡県古賀市",
	4022400: "福岡県福津市",
	4022500: "福岡県うきは市",
	4022600: "福岡県宮若市",
	4022700: "福岡県嘉麻市",
	4022800: "福岡県朝倉市",
	4022900: "福岡県みやま市",
	4023000: "福岡県糸島市",
	4023100: "福岡県那珂川市",
	4034100: "福岡県宇美町",
	4034200: "福岡県篠栗町",
	4034300: "福岡県志免町",
	4034400: "福岡県須恵町",
	4034500: "福岡県新宮町",
	4034800: "福岡県久山町",
	4034900: "福岡県粕屋町",
	4038100: "福岡県芦屋町",
	4038200: "福岡県水巻町",
	4038300: "福岡県岡垣町",
	4038400: "福岡県遠賀町",
	4040100: "福岡県小竹町",
	4040200: "福岡県鞍手町",
	4042100: "福岡県桂川町",
	4044700: "福岡県筑前町",
	4044800: "福岡県東峰村",
	4050300: "福岡県大刀洗町",
	4052200: "福岡県大木町",
	4054400: "福岡県広川町",
	4060100: "福岡県香春町",
	4060200: "福岡県添田町",
	4060400: "福岡県糸田町",
	4060500: "福岡県川崎町",
	4060800: "福岡県大任町",
	4060900: "福岡県赤村",
	4061000: "福岡県福智町",
	4062100: "福岡県苅田町",
	4062500: "福岡県みやこ町",
	4064200: "福岡県吉富町",
	4064600: "福岡県上毛町",
	4064700: "福岡県築上町",
	4099999: "福岡県のその他の市町村",
	4120100: "佐賀県佐賀市",
	4120200: "佐賀県唐津市",
	4120300: "佐賀県鳥栖市",
	4120400: "佐賀県多久市",
	4120500: "佐賀県伊万里市",
	4120600: "佐賀県武雄市",
	4120700: "佐賀県鹿島市",
	4120800: "佐賀県小城市",
	4120900: "佐賀県嬉野市",
	4121000: "佐賀県神埼市",
	4132700: "佐賀県吉野ヶ里町",
	4134100: "佐賀県基山町",
	4134500: "佐賀県上峰町",
	4134600: "佐賀県みやき町",
	4138700: "佐賀県玄海町",
	4140100: "佐賀県有田町",
	4142300: "佐賀県大町町",
	4142400: "佐賀県江北町",
	4142500: "佐賀県白石町",
	4144100: "佐賀県太良町",
	4199999: "佐賀県のその他の市町村",
	4220100: "長崎県長崎市",
	4220200: "長崎県佐世保市",
	4220300: "長崎県島原市",
	4220400: "長崎県諫早市",
	4220500: "長崎県大村市",
	4220700: "長崎県平戸市",
	4220800: "長崎県松浦市",
	4220900: "長崎県対馬市",
	4221000: "長崎県壱岐市",
	4221100: "長崎県五島市",
	4221200: "長崎県西海市",
	4221300: "長崎県雲仙市",
	4221400: "長崎県南島原市",
	4230700: "長崎県長与町",
	4230800: "長崎県時津町",
	4232100: "長崎県東彼杵町",
	4232200: "長崎県川棚町",
	4232300: "長崎県波佐見町",
	4238300: "長崎県小値賀町",
	4239100: "長崎県佐々町",
	4241100: "長崎県新上五島町",
	4299999: "長崎県のその他の市町村",
	4310000: "熊本県熊本市",
	4320200: "熊本県八代市",
	4320300: "熊本県人吉市",
	4320400: "熊本県荒尾市",
	4320500: "熊本県水俣市",
	4320600: "熊本県玉名市",
	4320800: "熊本県山鹿市",
	4321000: "熊本県菊池市",
	4321100: "熊本県宇土市",
	4321200: "熊本県上天草市",
	4321300: "熊本県宇城市",
	4321400: "熊本県阿蘇市",
	4321500: "熊本県天草市",
	4321600: "熊本県合志市",
	4334800: "熊本県美里町",
	4336400: "熊本県玉東町",
	4336700: "熊本県南関町",
	4336800: "熊本県長洲町",
	4336900: "熊本県和水町",
	4340300: "熊本県大津町",
	4340400: "熊本県菊陽町",
	4342300: "熊本県南小国町",
	4342400: "熊本県小国町",
	4342500: "熊本県産山村",
	4342800: "熊本県高森町",
	4343200: "熊本県西原村",
	4343300: "熊本県南阿蘇村",
	4344100: "熊本県御船町",
	4344200: "熊本県嘉島町",
	4344300: "熊本県益城町",
	4344400: "熊本県甲佐町",
	4344700: "熊本県山都町",
	4346800: "熊本県氷川町",
	4348200: "熊本県芦北町",
	4348400: "熊本県津奈木町",
	4350100: "熊本県錦町",
	4350500: "熊本県多良木町",
	4350600: "熊本県湯前町",
	4350700: "熊本県水上村",
	4351000: "熊本県相良村",
	4351100: "熊本県五木村",
	4351200: "熊本県山江村",
	4351300: "熊本県球磨村",
	4351400: "熊本県あさぎり町",
	4353100: "熊本県苓北町",
	4399999: "熊本県のその他の市町村",
	4420100: "大分県大分市",
	4420200: "大分県別府市",
	4420300: "大分県中津市",
	4420400: "大分県日田市",
	4420500: "大分県佐伯市",
	4420600: "大分県臼杵市",
	4420700: "大分県津久見市",
	4420800: "大分県竹田市",
	4420900: "大分県豊後高田市",
	4421000: "大分県杵築市",
	4421100: "大分県宇佐市",
	4421200: "大分県豊後大野市",
	4421300: "大分県由布市",
	4421400: "大分県国東市",
	4432200: "大分県姫島村",
	4434100: "大分県日出町",
	4446100: "大分県九重町",
	4446200: "大分県玖珠町",
	4499999: "大分県のその他の市町村",
	4520100: "宮崎県宮崎市",
	4520200: "宮崎県都城市",
	4520300: "宮崎県延岡市",
	4520400: "宮崎県日南市",
	4520500: "宮崎県小林市",
	4520600: "宮崎県日向市",
	4520700: "宮崎県串間市",
	4520800: "宮崎県西都市",
	4520900: "宮崎県えびの市",
	4534100: "宮崎県三股町",
	4536100: "宮崎県高原町",
	4538200: "宮崎県国富町",
	4538300: "宮崎県綾町",
	4540100: "宮崎県高鍋町",
	4540200: "宮崎県新富町",
	4540300: "宮崎県西米良村",
	4540400: "宮崎県木城町",
	4540500: "宮崎県川南町",
	4540600: "宮崎県都農町",
	4542100: "宮崎県門川町",
	4542900: "宮崎県諸塚村",
	4543000: "宮崎県椎葉村",
	4543100: "宮崎県美郷町",
	4544100: "宮崎県高千穂町",
	4544200: "宮崎県日之影町",
	4544300: "宮崎県五ヶ瀬町",
	4599999: "宮崎県のその他の市町村",
	4620100: "鹿児島県鹿児島市",
	4620300: "鹿児島県鹿屋市",
	4620400: "鹿児島県枕崎市",
	4620600: "鹿児島県阿久根市",
	4620800: "鹿児島県出水市",
	4621000: "鹿児島県指宿市",
	4621300: "鹿児島県西之表市",
	4621400: "鹿児島県垂水市",
	4621500: "鹿児島県薩摩川内市",
	4621600: "鹿児島県日置市",
	4621700: "鹿児島県曽於市",
	4621800: "鹿児島県霧島市",
	4621900: "鹿児島県いちき串木野市",
	4622000: "鹿児島県南さつま市",
	4622100: "鹿児島県志布志市",
	4622200: "鹿児島県奄美市",
	4622300: "鹿児島県南九州市",
	4622400: "鹿児島県伊佐市",
	4622500: "鹿児島県姶良市",
	4630300: "鹿児島県三島村",
	4630400: "鹿児島県十島村",
	4639200: "鹿児島県さつま町",
	4640400: "鹿児島県長島町",
	4645200: "鹿児島県湧水町",
	4646800: "鹿児島県大崎町",
	4648200: "鹿児島県東串良町",
	4649000: "鹿児島県錦江町",
	4649100: "鹿児島県南大隅町",
	4649200: "鹿児島県肝付町",
	4650100: "鹿児島県中種子町",
	4650200: "鹿児島県南種子町",
	4650500: "鹿児島県屋久島町",
	4652300: "鹿児島県大和村",
	4652400: "鹿児島県宇検村",
	4652500: "鹿児島県瀬戸内町",
	4652700: "鹿児島県龍郷町",
	4652900: "鹿児島県喜界町",
	4653000: "鹿児島県徳之島町",
	4653100: "鹿児島県天城町",
	4653200: "鹿児島県伊仙町",
	4653300: "鹿児島県和泊町",
	4653400: "鹿児島県知名町",
	4653500: "鹿児島県与論町",
	4699999: "鹿児島県のその他の市町村",
	4720100: "沖縄県那覇市",
	4720500: "沖縄県宜野湾市",
	4720700: "沖縄県石垣市",
	4720800: "沖縄県浦添市",
	4720900: "沖縄県名護市",
	4721000: "沖縄県糸満市",
	4721100: "沖縄県沖縄市",
	4721200: "沖縄県豊見城市",
	4721300: "沖縄県うるま市",
	4721400: "沖縄県宮古島市",
	4721500: "沖縄県南城市",
	4730100: "沖縄県国頭村",
	4730200: "沖縄県大宜味村",
	4730300: "沖縄県東村",
	4730600: "沖縄県今帰仁村",
	4730800: "沖縄県本部町",
	4731100: "沖縄県恩納村",
	4731300: "沖縄県宜野座村",
	4731400: "沖縄県金武町",
	4731500: "沖縄県伊江村",
	4732400: "沖縄県読谷村",
	4732500: "沖縄県嘉手納町",
	4732600: "沖縄県北谷町",
	4732700: "沖縄県北中城村",
	4732800: "沖縄県中城村",
	4732900: "沖縄県西原町",
	4734800: "沖縄県与那原町",
	4735000: "沖縄県南風原町",
	4735300: "沖縄県渡嘉敷村",
	4735400: "沖縄県座間味村",
	4735500: "沖縄県粟国村",
	4735600: "沖縄県渡名喜村",
	4735700: "沖縄県南大東村",
	4735800: "沖縄県北大東村",
	4735900: "沖縄県伊平屋村",
	4736000: "沖縄県伊是名村",
	4736100: "沖縄県久米島町",
	4736200: "沖縄県八重瀬町",
	4737500: "沖縄県多良間村",
	4738100: "沖縄県竹富町",
	4738200: "沖縄県与那国町",
	4799999: "沖縄県のその他の市町村"
}

matchtable_mt43_volcano_undefined_Pl = { #33 undefined ones
    '1': '北海道の市町村(コード番号：1NNNNN)',
    '2': '青森県の市町村(コード番号：2NNNNN)',
    '3': '岩手県の市町村(コード番号：3NNNNN)',
    '4': '宮城県の市町村(コード番号：4NNNNN)',
    '5': '秋田県の市町村(コード番号：5NNNNN)',
    '6': '山形県の市町村(コード番号：6NNNNN)',
    '7': '福島県の市町村(コード番号：7NNNNN)',
    '8': '茨城県の市町村(コード番号：8NNNNN)',
    '9': '栃木県の市町村(コード番号：9NNNNN)',
    '10': '群馬県の市町村(コード番号：10NNNNN)',
    '11': '埼玉県の市町村(コード番号：11NNNNN)',
    '12': '千葉県の市町村(コード番号：12NNNNN)',
    '13': '東京都の市町村(コード番号：13NNNNN)',
    '14': '神奈川県の市町村(コード番号：14NNNNN)',
    '15': '新潟県の市町村(コード番号：15NNNNN)',
    '16': '富山県の市町村(コード番号：16NNNNN)',
    '17': '石川県の市町村(コード番号：17NNNNN)',
    '18': '福井県の市町村(コード番号：18NNNNN)',
    '19': '山梨県の市町村(コード番号：19NNNNN)',
    '20': '長野県の市町村(コード番号：20NNNNN)',
    '21': '岐阜県の市町村(コード番号：21NNNNN)',
    '22': '静岡県の市町村(コード番号：22NNNNN)',
    '23': '愛知県の市町村(コード番号：23NNNNN)',
    '24': '三重県の市町村(コード番号：24NNNNN)',
    '25': '滋賀県の市町村(コード番号：25NNNNN)',
    '26': '京都府の市町村(コード番号：26NNNNN)',
    '27': '大阪府の市町村(コード番号：27NNNNN)',
    '28': '兵庫県の市町村(コード番号：28NNNNN)',
    '29': '奈良県の市町村(コード番号：29NNNNN)',
    '30': '和歌山県の市町村(コード番号：30NNNNN)',
    '31': '鳥取県の市町村(コード番号：31NNNNN)',
    '32': '島根県の市町村(コード番号：32NNNNN)',
    '33': '岡山県の市町村(コード番号：33NNNNN)',
    '34': '広島県の市町村(コード番号：34NNNNN)',
    '35': '山口県の市町村(コード番号：35NNNNN)',
    '36': '徳島県の市町村(コード番号：36NNNNN)',
    '37': '香川県の市町村(コード番号：37NNNNN)',
    '38': '愛媛県の市町村(コード番号：38NNNNN)',
    '39': '高知県の市町村(コード番号：39NNNNN)',
    '40': '福岡県の市町村(コード番号：40NNNNN)',
    '41': '佐賀県の市町村(コード番号：41NNNNN)',
    '42': '長崎県の市町村(コード番号：42NNNNN)',
    '43': '熊本県の市町村(コード番号：43NNNNN)',
    '44': '大分県の市町村(コード番号：44NNNNN)',
    '45': '宮崎県の市町村(コード番号：45NNNNN)',
    '46': '鹿児島県の市町村(コード番号：46NNNNN)',
    '47': '沖縄県の市町村(コード番号：47NNNNN)'    
}

def get_mt43_volcano_Pl(Pl):
    Pl_dec = matchtable_mt43_volcano_Pl.get(Pl, "")
    if Pl_dec == "":
        denominator = Pl // 100000
        Pl_dec = matchtable_mt43_volcano_undefined_Pl.get(denominator, "")
    return Pl_dec
    
#----------
matchtable_mt43_ashfall_Dw2 = { #36
    1: '少量の降灰',
    2: 'やや多量の降灰',
    3: '多量の降灰',
    4: '小さな噴石の落下',
    7: 'その他の防災気象情報要素'                                                       
}

def get_mt43_ashfall_Dw2(Dw2):
    Dw2_dec = matchtable_mt43_ashfall_Dw2.get(Dw2, "防災気象情報要素 2(コード番号：N)")
    return Dw2_dec

#----------
matchtable_mt43_weather_Ar = { #39
    1: "発表",
    2: "解除"                          
}

def get_mt43_weather_Ar(Ar):
    Ar_dec = matchtable_mt43_weather_Ww.get(Ar, "発表状況(コード番号：N)")
    return Ar_dec

matchtable_mt43_weather_Ww = { #40
    1: '暴風雪特別警報',
    2: '大雨特別警報',
    3: '暴風特別警報',
    4: '大雪特別警報',
    5: '波浪特別警報',
    6: '高潮特別警報',
    7: '全ての気象特別警報',
    21: '記録的短時間大雨情報',
    22: '竜巻注意情報',
    23: '土砂災害警戒情報',
    31: 'その他の警報等情報要素'                              
}

def get_mt43_weather_Ww(Ww):
    Ww_dec = matchtable_mt43_weather_Ww.get(Ww, "警報等情報要素(コード番号：NN)")
    return Ww_dec

matchtable_mt43_weather_Pl = { #41
    11000: '宗谷地方',
    12000: '上川・留萌地方',
    12010: '上川地方',
    12020: '留萌地方',
    13000: '網走・北見・紋別地方',
    14010: '根室地方',
    14020: '釧路地方',
    14030: '十勝地方',
    14100: '釧路・根室地方',
    15000: '胆振・日高地方',
    15010: '胆振地方',
    15020: '日高地方',
    16000: '石狩・空知・後志地方',
    16010: '石狩地方',
    16020: '空知地方',
    16030: '後志地方',
    16100: '石狩・空知地方',
    17000: '渡島・檜山地方',
    17010: '渡島地方',
    17020: '檜山地方',
    20000: '青森県',
    30000: '岩手県',
    40000: '宮城県',
    50000: '秋田県',
    60000: '山形県',
    70000: '福島県',
    80000: '茨城県',
    90000: '栃木県',
    100000: '群馬県',
    110000: '埼玉県',
    120000: '千葉県',
    130000: '東京都',
    130010: '東京地方',
    130020: '伊豆諸島北部',
    130030: '伊豆諸島南部',
    140000: '神奈川県',
    150000: '新潟県',
    160000: '富山県',
    170000: '石川県',
    180000: '福井県',
    190000: '山梨県',
    200000: '長野県',
    210000: '岐阜県',
    220000: '静岡県',
    230000: '愛知県',
    240000: '三重県',
    250000: '滋賀県',
    260000: '京都府',
    270000: '大阪府',
    280000: '兵庫県',
    290000: '奈良県',
    300000: '和歌山県',
    310000: '鳥取県',
    320000: '島根県',
    330000: '岡山県',
    340000: '広島県',
    350000: '山口県',
    360000: '徳島県',
    370000: '香川県',
    380000: '愛媛県',
    390000: '高知県',
    400000: '福岡県',
    410000: '佐賀県',
    420000: '長崎県',
    430000: '熊本県',
    440000: '大分県',
    450000: '宮崎県',
    460000: '鹿児島県',
    460040: '奄美地方',
    460100: '鹿児島県(奄美地方除く)',
    471000: '沖縄本島地方',
    472000: '大東島地方',
    473000: '宮古島地方',
    474000: '八重山地方',
    500000: 'その他の府県予報区'                              
}

def get_mt43_weather_Pl(Pl):
    Pl_dec = matchtable_mt43_weather_Pl.get(Pl, "府県予報区(コード番号：NNNNNN)")
    return Pl_dec
                              
#----------
matchtable_mt43_flood_Lv = { #44 
    1: '警報解除',
    2: '氾濫警戒情報',
    3: '氾濫危険情報',
    4: '氾濫発生情報',
    15: 'その他の警戒レベル'                                                        
}

def get_mt43_flood_Lv(Lv):
    Lv_dec = matchtable_mt43_flood_Lv.get(Lv, "警戒レベル(コード番号：NN)")
    return Lv_dec

matchtable_mt43_flood_Pl = { #45 #long later add                            
 	10175000100: "新川(北海道道央)",
	19999999999: "北海道のその他の河川",
	20036000100: "堤川・駒込川(青森県)",
	29999999999: "青森県のその他の河川",
	39999999999: "岩手県のその他の河川",
	40028000100: "七北田川(宮城県)",
	49999999999: "宮城県のその他の河川",
	59999999999: "秋田県のその他の河川",
	60016000100: "日向川(山形県)",
	69999999999: "山形県のその他の河川",
	70006000100: "宇多川(福島県)",
	70010000100: "新田川(福島県)",
	70028000100: "夏井川(福島県)",
	79999999999: "福島県のその他の河川",
	89999999999: "茨城県のその他の河川",
	99999999999: "栃木県のその他の河川",
	109999999999: "群馬県のその他の河川",
	119999999999: "埼玉県のその他の河川",
	129999999999: "千葉県のその他の河川",
	130005000100: "目黒川(東京都)",
	130006000100: "渋谷川・古川(東京都)",
	139999999999: "東京都のその他の河川",
	140019000100: "酒匂川(神奈川県)",
	149999999999: "神奈川県のその他の河川",
	159999999999: "新潟県のその他の河川",
	169999999999: "富山県のその他の河川",
	179999999999: "石川県のその他の河川",
	180006000100: "南川(福井県)",
	180014000100: "笙の川(福井県)",
	189999999999: "福井県のその他の河川",
	199999999999: "山梨県のその他の河川",
	209999999999: "長野県のその他の河川",
	219999999999: "岐阜県のその他の河川",
	220062000100: "瀬戸川・朝比奈川(静岡県)",
	220079000100: "太田川・原野谷川(静岡県)",
	220081000100: "都田川(静岡県)",
	229999999999: "静岡県のその他の河川",
	230029000100: "境川・逢妻川(愛知県)",
	230052000100: "天白川(愛知県)",
	230054000100: "日光川(愛知県)",
	239999999999: "愛知県のその他の河川",
	249999999999: "三重県のその他の河川",
	259999999999: "滋賀県のその他の河川",
	269999999999: "京都府のその他の河川",
	270013000100: "大津川・槇尾川(大阪府)",
	270013000200: "牛滝川(大阪府)",
	279999999999: "大阪府のその他の河川",
	280014000100: "武庫川(兵庫県)",
	280053000100: "市川(兵庫県)",
	280064000100: "千種川(兵庫県)",
	289999999999: "兵庫県のその他の河川",
	299999999999: "奈良県のその他の河川",
	300013000100: "古座川(和歌山県)",
	300066000100: "日高川(和歌山県)",
	300077000100: "有田川(和歌山県)",
	309999999999: "和歌山県のその他の河川",
	310027000100: "由良川(鳥取県)",
	319999999999: "鳥取県のその他の河川",
	320004000100: "益田川(島根県)",
	320011000100: "周布川(島根県)",
	329999999999: "島根県のその他の河川",
	330009000100: "笹ヶ瀬川・足守川(岡山県)",
	339999999999: "岡山県のその他の河川",
	340013000100: "沼田川(広島県)",
	340021000100: "黒瀬川(広島県)",
	349999999999: "広島県のその他の河川",
	350002000102: "錦川下流部(山口県)",
	350002000103: "錦川中流部(山口県)",
	350019000100: "島田川(山口県)",
	350036000100: "椹野川(山口県)",
	350036001900: "仁保川(山口県)",
	350046000100: "厚東川(山口県)",
	359999999999: "山口県のその他の河川",
	360007000100: "勝浦川(徳島県)",
	369999999999: "徳島県のその他の河川",
	370026000100: "香東川(香川県)",
	379999999999: "香川県のその他の河川",
	389999999999: "愛媛県のその他の河川",
	399999999999: "高知県のその他の河川",
	400018000100: "御笠川(福岡県)",
	409999999999: "福岡県のその他の河川",
	419999999999: "佐賀県のその他の河川",
	429999999999: "長崎県のその他の河川",
	439999999999: "熊本県のその他の河川",
	440008000100: "駅館川(大分県)",
	449999999999: "大分県のその他の河川",
	450026000100: "清武川(宮崎県)",
	450040000100: "広渡川(宮崎県)",
	450040000700: "酒谷川(宮崎県)",
	459999999999: "宮崎県のその他の河川",
	460060000100: "万之瀬川・加世田川(鹿児島県)",
	469999999999: "鹿児島県のその他の河川",
	479999999999: "沖縄県のその他の河川",
	809999999999: "沖縄県のその他の河川",
	810101000100: "天塩川(北海道道北)",
	810101008500: "名寄川(北海道道北)",
	810102000100: "留萌川(北海道道北)",
	810103000101: "石狩川上流(北海道道北)",
	810103000102: "石狩川下流(北海道道央)",
	810103003100: "豊平川(北海道道央)",
	810103008100: "千歳川(北海道道央)",
	810103011900: "夕張川(北海道道央)",
	810103018200: "幾春別川(北海道道央)",
	810103025001: "空知川上流(北海道道北)",
	810103025002: "空知川下流(北海道道央)",
	810103033400: "雨竜川(北海道道央)",
	810103040900: "忠別川(北海道道北)",
	810103041000: "美瑛川(北海道道北)",
	810103044200: "牛朱別川(北海道道北)",
	810104000100: "尻別川(北海道道央)",
	810105000100: "後志利別川(北海道道南)",
	810106000100: "鵡川(北海道道南)",
	810107000100: "沙流川(北海道道南)",
	810108000100: "十勝川(北海道道東)",
	810108003400: "利別川(北海道道東)",
	810108011400: "札内川(北海道道東)",
	810108013900: "音更川(北海道道東)",
	810109000100: "釧路川・新釧路川(北海道道東)",
	810110000100: "網走川(北海道道東)",
	810110001000: "美幌川(北海道道東)",
	810111000100: "常呂川(北海道道東)",
	810111002400: "無加川(北海道道東)",
	810112000100: "湧別川(北海道道東)",
	810113000100: "渚滑川(北海道道東)",
	819999999999: "北海道のその他の河川",
	820201000101: "阿武隈川上流(福島県・宮城県)",
	820201000102: "阿武隈川下流(宮城県)",
	820201000400: "白石川(宮城県)",
	820201009900: "荒川(福島県)",
	820202000100: "名取川(宮城県)",
	820202001100: "広瀬川(宮城県)",
	820203000100: "鳴瀬川(宮城県)",
	820203000400: "吉田川(宮城県)",
	820204000101: "北上川上流(岩手県)",
	820204000102: "北上川下流(宮城県)",
	820204001800: "旧北上川(宮城県)",
	820204001900: "迫川(宮城県)",
	820204007100: "江合川(宮城県)",
	820204035600: "猿ヶ石川(岩手県)",
	820204048200: "雫石川(岩手県)",
	820205000102: "馬淵川下流(青森県)",
	820205000103: "馬淵川中流(青森県)",
	820206000100: "高瀬川(小川原湖)(青森県)",
	820207000100: "岩木川(青森県)",
	820207003500: "十川(青森県)",
	820207006101: "平川上流(青森県)",
	820207006102: "平川下流(青森県)",
	820208000100: "米代川(秋田県)",
	820209000101: "雄物川上流(秋田県)",
	820209000102: "雄物川下流(秋田県)",
	820209000400: "太平川(秋田県)",
	820209005500: "玉川(秋田県)",
	820209018700: "皆瀬川(秋田県)",
	820210000100: "子吉川(秋田県)",
	820211000101: "最上川上流(山形県)",
	820211000102: "最上川下流(山形県)",
	820211000103: "最上川中流(山形県)",
	820211007300: "鮭川(山形県)",
	820211017000: "最上小国川(山形県)",
	820211022100: "丹生川(山形県)",
	820211030701: "須川上流(山形県)",
	820211030702: "須川下流(山形県)",
	820211048700: "屋代川(山形県)",
	820212000100: "赤川(山形県)",
	820212000200: "大山川(山形県)",
	829999999999: "東北地方のその他の河川",
	830301000100: "久慈川(茨城県)",
	830302000100: "那珂川(栃木県・茨城県)",
	830302000101: "那珂川上流部(栃木県)",
	830302008300: "逆川(栃木県)",
	830302009400: "荒川(栃木県)",
	830302014300: "箒川(栃木県)",
	830302014700: "蛇尾川(栃木県)",
	830302018300: "余笹川(栃木県)",
	830303000101: "利根川上流部(群馬県・茨城県・千葉県・埼玉県)",
	830303000102: "利根川下流部(茨城県・千葉県)",
	830303000103: "利根川中流部(茨城県・千葉県)",
	830303002800: "霞ヶ浦・北浦(茨城県・千葉県)",
	830303004600: "桜川(茨城県)",
	830303015200: "小貝川(栃木県・茨城県)",
	830303015201: "小貝川上流部(栃木県)",
	830303017900: "五行川(栃木県)",
	830303020300: "鬼怒川(栃木県・茨城県)",
	830303021100: "田川(栃木県)",
	830303030400: "江戸川(茨城県・埼玉県・東京都・千葉県)",
	830303033900: "中川(埼玉県・東京都)",
	830303043600: "綾瀬川(谷古宇区間)(埼玉県・東京都)",
	830303043603: "綾瀬川中流部(一の橋区間)(埼玉県)",
	830303046101: "渡良瀬川上流部(群馬県・栃木県)",
	830303046102: "渡良瀬川下流部(栃木県・茨城県・群馬県)",
	830303048000: "思川(栃木県)",
	830303048100: "姿川(栃木県)",
	830303049400: "黒川(栃木県)",
	830303051700: "永野川(栃木県)",
	830303053000: "秋山川(栃木県)",
	830303054800: "袋川(栃木県)",
	830303060000: "石田川(群馬県・埼玉県)",
	830303068900: "烏川流域(群馬県・埼玉県)",
	830303069600: "神流川(群馬県・埼玉県)",
	830304000100: "荒川(埼玉県・東京都)",
	830304000300: "芝川・新芝川(埼玉県・東京都)",
	830304001200: "新河岸川(埼玉県)",
	830304003200: "石神井川(東京都)",
	830304004400: "神田川(東京都)",
	830304004700: "妙正寺川(東京都)",
	830304006400: "入間川流域(埼玉県)",
	830305000100: "多摩川(東京都・神奈川県)",
	830305000500: "野川・仙川(東京都)",
	830305002000: "浅川(東京都)",
	830305002000: "浅川(東京都)",
	830306000100: "鶴見川(神奈川県)",
	830307000102: "相模川下流(神奈川県)",
	830307000103: "相模川中流(神奈川県)",
	830308000100: "富士川(釜無川を含む)(山梨県・静岡県)",
	830308039500: "笛吹川(山梨県)",
	830308045500: "荒川(山梨県)",
	830308065700: "塩川(山梨県)",
	839999999999: "関東地方のその他の河川",
	840401000100: "荒川(新潟県)",
	840402000100: "阿賀野川(新潟県)",
	840402000101: "阿賀川(福島県)",
	840403000100: "千曲川(長野県)",
	840403000101: "千曲川上流(長野県)",
	840403000102: "信濃川下流・中ノ口川(新潟県)",
	840403000103: "信濃川中流(新潟県)",
	840403023700: "魚野川(新潟県)",
	840403070500: "犀川(長野県)",
	840403070600: "裾花川(長野県)",
	840403089800: "奈良井川(長野県)",
	840404000100: "関川(新潟県)",
	840405000100: "姫川(新潟県)",
	840406000100: "黒部川(富山県)",
	840407000100: "常願寺川(富山県)",
	840408000101: "宮川(岐阜県)",
	840408000102: "神通川(富山県)",
	840409000100: "庄川(富山県)",
	840410000100: "小矢部川(富山県)",
	840411000100: "手取川(石川県)",
	840412000100: "梯川(石川県)",
	849999999999: "北陸地方のその他の河川",
	850501000100: "狩野川(静岡県)",
	850502000100: "安倍川(静岡県)",
	850503000100: "大井川(静岡県)",
	850504000100: "菊川(静岡県)",
	850505000101: "天竜川上流(長野県)",
	850505000102: "天竜川下流(静岡県)",
	850505071753: "諏訪湖(長野県)",
	850506000100: "豊川及び豊川放水路(愛知県)",
	850507000100: "矢作川(愛知県)",
	850508000100: "庄内川(岐阜県・愛知県)",
	850508000200: "新川(愛知県)",
	850509000102: "木曽川下流(愛知県・岐阜県・三重県)",
	850509000103: "木曽川中流(岐阜県・愛知県)",
	850509000202: "揖斐川下流(岐阜県・三重県)",
	850509000203: "揖斐川中流(岐阜県)",
	850509000401: "長良川上流(岐阜県)",
	850509000402: "長良川下流(岐阜県・愛知県・三重県)",
	850509000403: "長良川中流(岐阜県)",
	850509025500: "飛騨川(岐阜県)",
	850510000100: "鈴鹿川及び鈴鹿川派川(三重県)",
	850511000100: "雲出川及び雲出古川(三重県)",
	850512000100: "櫛田川(三重県)",
	850513000100: "宮川(三重県)",
	859999999999: "中部地方のその他の河川",
	860601000102: "熊野川下流(三重県・和歌山県)",
	860601000103: "熊野川中流(本宮区間)(和歌山県)",
	860601000105: "熊野川中流(日足区間)(和歌山県)",
	860602000100: "紀の川(奈良県・和歌山県)",
	860603000101: "大和川上流(奈良県・大阪府)",
	860603000102: "大和川下流(奈良県・大阪府)",
	860603001300: "石川(大阪府)",
	860604000101: "瀬田川(滋賀県)",
	860604000102: "淀川(京都府・大阪府)",
	860604000103: "宇治川(京都府・大阪府)",
	860604000600: "寝屋川流域(大阪府)",
	860604004700: "神崎川・安威川(大阪府)",
	860604007300: "猪名川(大阪府・兵庫県)",
	860604016702: "桂川下流(京都府・大阪府)",
	860604016703: "桂川中流・園部川(京都府)",
	860604018200: "鴨川・高野川(京都府)",
	860604037101: "木津川上流(三重県・京都府)",
	860604037102: "木津川下流(京都府)",
	860604051600: "名張川(三重県・奈良県)",
	860604086300: "琵琶湖(滋賀県)",
	860604101200: "姉川(滋賀県)",
	860604101300: "高時川(滋賀県)",
	860604120300: "日野川(滋賀県)",
	860604127501: "野洲川上流(滋賀県)",
	860604127502: "野洲川下流(滋賀県)",
	860604131100: "杣川(滋賀県)",
	860605000101: "加古川上流(兵庫県)",
	860605000102: "加古川下流(兵庫県)",
	860606000101: "揖保川上流(兵庫県)",
	860606000102: "揖保川下流(兵庫県)",
	860607000100: "九頭竜川(福井県)",
	860607000200: "竹田川(福井県)",
	860607001902: "日野川下流(福井県)",
	860607001903: "日野川中流(福井県)",
	860607002200: "足羽川(福井県)",
	860608000100: "北川(福井県)",
	860609000102: "由良川下流(京都府)",
	860609000103: "由良川中流(京都府)",
	860610000100: "円山川(兵庫県)",
	860610002600: "出石川(兵庫県)",
	869999999999: "近畿地方のその他の河川",
	870701000100: "千代川(鳥取県)",
	870701000900: "袋川・新袋川(鳥取県)",
	870702000100: "天神川(鳥取県)",
	870702000300: "小鴨川(鳥取県)",
	870702000400: "国府川(鳥取県)",
	870703000100: "日野川(鳥取県)",
	870703000300: "法勝寺川(鳥取県)",
	870704000100: "斐伊川(島根県)",
	870704002800: "飯梨川(島根県)",
	870704024400: "神戸川(島根県)",
	870705000101: "江の川上流(広島県・島根県)",
	870705000102: "江の川下流(広島県・島根県)",
	870705015700: "神野瀬川(広島県)",
	870705016300: "馬洗川(広島県)",
	870705016400: "西城川(広島県)",
	870706000100: "高津川(島根県)",
	870706001700: "匹見川(島根県)",
	870707000100: "佐波川(山口県)",
	870708000100: "小瀬川(広島県・山口県)",
	870709000101: "太田川上流(広島県)",
	870709000102: "太田川下流(広島県)",
	870709002900: "三篠川(広島県)",
	870709004700: "根谷川(広島県)",
	870710000100: "芦田川(広島県)",
	870710001100: "高屋川(広島県)",
	870711000100: "高梁川(岡山県)",
	870711000500: "小田川(岡山県)",
	870712000100: "旭川(岡山県)",
	870712000103: "旭川水系旭川中流部(岡山県)",
	870712000400: "百間川(岡山県)",
	870713000100: "吉井川(岡山県)",
	870713002800: "金剛川(岡山県)",
	879999999999: "中国地方のその他の河川",
	880801000100: "重信川(愛媛県)",
	880802000100: "肱川(愛媛県)",
	880803000100: "四万十川(高知県)",
	880804000100: "仁淀川(高知県)",
	880805000100: "物部川(高知県)",
	880806000100: "那賀川(徳島県)",
	880807000100: "吉野川(徳島県)",
	880808000100: "土器川(香川県)",
	889999999999: "四国地方のその他の河川",
	890901000101: "遠賀川上流部(福岡県)",
	890901000102: "遠賀川下流部(福岡県)",
	890901010300: "彦山川(福岡県)",
	890902000100: "松浦川(佐賀県)",
	890902001300: "徳須恵川(佐賀県)",
	890902004900: "厳木川(佐賀県)",
	890903000100: "本明川(長崎県)",
	890904000100: "六角川(佐賀県)",
	890904000300: "牛津川(佐賀県)",
	890905000100: "嘉瀬川(佐賀県)",
	890906000102: "筑後川下流部(福岡県・佐賀県)",
	890906000104: "筑後川上中流部(大分県・福岡県)",
	890907000100: "矢部川(福岡県)",
	890908000100: "菊池川水系(熊本県)",
	890909000100: "白川(熊本県)",
	890910000100: "緑川水系(熊本県)",
	890911000100: "球磨川(熊本県)",
	890912000101: "川内川上流部(宮崎県・鹿児島県)",
	890912000102: "川内川下流部(鹿児島県)",
	890913000100: "肝属川水系(鹿児島県)",
	890914000101: "大淀川上流部(宮崎県)",
	890914000102: "大淀川下流部(宮崎県)",
	890914003300: "本庄川(宮崎県)",
	890915000100: "小丸川(宮崎県)",
	890916000100: "五ヶ瀬川・大瀬川(宮崎県)",
	890917000100: "番匠川(大分県)",
	890918000100: "大野川水系(大分県)",
	890919000100: "大分川(大分県)",
	890919001100: "七瀬川(大分県)",
	890920000101: "山国川上流部(大分県・福岡県)",
	890920000102: "山国川下流部(大分県・福岡県)",
	899999999999: "九州地方のその他の河川"                     
}

matchtable_mt43_flood_undefined_Pl = { #45 undefined                                       
	1: "北海道の河川(コード番号：1NNNNNNNNNN)",
	2: "青森県の河川(コード番号：2NNNNNNNNNN)",
	3: "岩手県の河川(コード番号：3NNNNNNNNNN)",
	4: "宮城県の河川(コード番号：4NNNNNNNNNN)",
	5: "秋田県の河川(コード番号：5NNNNNNNNNN)",
	6: "山形県の河川(コード番号：6NNNNNNNNNN)",
	7: "福島県の河川(コード番号：7NNNNNNNNNN)",
	8: "茨城県の河川(コード番号：8NNNNNNNNNN)",
	9: "栃木県の河川(コード番号：9NNNNNNNNNN)",
	10: "群馬県の河川(コード番号：10NNNNNNNNNN)",
	11: "埼玉県の河川(コード番号：11NNNNNNNNNN)",
	12: "千葉県の河川(コード番号：12NNNNNNNNNN)",
	13: "東京都の河川(コード番号：13NNNNNNNNNN)",
	14: "神奈川県の河川(コード番号：14NNNNNNNNNN)",
	15: "新潟県の河川(コード番号：15NNNNNNNNNN)",
	16: "富山県の河川(コード番号：16NNNNNNNNNN)",
	17: "石川県の河川(コード番号：17NNNNNNNNNN)",
	18: "福井県の河川(コード番号：18NNNNNNNNNN)",
	19: "山梨県の河川(コード番号：19NNNNNNNNNN)",
	20: "長野県の河川(コード番号：20NNNNNNNNNN)",
	21: "岐阜県の河川(コード番号：21NNNNNNNNNN)",
	22: "静岡県の河川(コード番号：22NNNNNNNNNN)",
	23: "愛知県の河川(コード番号：23NNNNNNNNNN)",
	24: "三重県の河川(コード番号：24NNNNNNNNNN)",
	25: "滋賀県の河川(コード番号：25NNNNNNNNNN)",
	26: "京都府の河川(コード番号：26NNNNNNNNNN)",
	27: "大阪府の河川(コード番号：27NNNNNNNNNN)",
	28: "兵庫県の河川(コード番号：28NNNNNNNNNN)",
	29: "奈良県の河川(コード番号：29NNNNNNNNNN)",
	30: "和歌山県の河川(コード番号：30NNNNNNNNNN)",
	31: "鳥取県の河川(コード番号：31NNNNNNNNNN)",
	32: "島根県の河川(コード番号：32NNNNNNNNNN)",
	33: "岡山県の河川(コード番号：33NNNNNNNNNN)",
	34: "広島県の河川(コード番号：34NNNNNNNNNN)",
	35: "山口県の河川(コード番号：35NNNNNNNNNN)",
	36: "徳島県の河川(コード番号：36NNNNNNNNNN)",
	37: "香川県の河川(コード番号：37NNNNNNNNNN)",
	38: "愛媛県の河川(コード番号：38NNNNNNNNNN)",
	39: "高知県の河川(コード番号：39NNNNNNNNNN)",
	40: "福岡県の河川(コード番号：40NNNNNNNNNN)",
	41: "佐賀県の河川(コード番号：41NNNNNNNNNN)",
	42: "長崎県の河川(コード番号：42NNNNNNNNNN)",
	43: "熊本県の河川(コード番号：43NNNNNNNNNN)",
	44: "大分県の河川(コード番号：44NNNNNNNNNN)",
	45: "宮崎県の河川(コード番号：45NNNNNNNNNN)",
	46: "鹿児島県の河川(コード番号：46NNNNNNNNNN)",
	47: "沖縄県の河川(コード番号：47NNNNNNNNNN)",
	48: "沖縄県の河川(コード番号：80NNNNNNNNNN)",
	49: "北海道の河川(コード番号：81NNNNNNNNNN)",
	50: "東北地方の河川(コード番号：82NNNNNNNNNN)",
	51: "関東地方の河川(コード番号：83NNNNNNNNNN)",
	52: "北陸地方の河川(コード番号：84NNNNNNNNNN)",
	53: "中部地方の河川(コード番号：85NNNNNNNNNN)",
	54: "近畿地方の河川(コード番号：86NNNNNNNNNN)",
	55: "中国地方の河川(コード番号：87NNNNNNNNNN)",
	56: "四国地方の河川(コード番号：88NNNNNNNNNN)",
	57: "九州地方の河川(コード番号：89NNNNNNNNNN)"
}
#----------

def get_mt43_flood_Pl(Pl):
    Pl_dec = matchtable_mt43_flood_Pl.get(Pl, "")
    if Pl_dec == "":
        denominator = Pl // 10000000000
        Pl_dec = matchtable_mt43_flood_undefined_Pl.get(denominator, "")
    return Pl_dec

#-----------

matchtable_mt43_typhoon_Sr = { #48
    0: 'なし',
    1: '大型',
    2: '超大型',
    15: 'その他の大きさ階級分類'                                  
}

def get_mt43_typhoon_Sr(Sr):
    Sr_dec = matchtable_mt43_typhoon_Sr.get(Sr, "大きさ階級分類(コード番号：NN)")
    return Sr_dec
    
matchtable_mt43_typhoon_Ic = { #49
    0: 'なし',
    1: '強い',
    2: '非常に強い',
    3: '猛烈な',
    15: 'その他の強さ階級分類'                                                            
}


def get_mt43_typhoon_Ic(Ic):
    Ic_dec = matchtable_mt43_typhoon_Ic.get(Ic, "強さ階級分類(コード番号：NN)")
    return Ic_dec

#----------
matchtable_mt43_marine_Dw = {
    0: "海上警報解除",
    10: "海上着氷警報",
    11: "海上濃霧警報",
    12: "海上うねり警報",
    20: "海上風警報",
    21: "海上強風警報",
    22: "海上暴風警報",
    23: "海上台風警報",
    31: "その他の警報等情報要素_海上警報",
}

def get_mt43_marine_Dw(Dw):
    return matchtable_mt43_marine_Dw.get(Dw, "警報等情報要素_海上警報(コード番号：NN)")

matchtable_mt43_marine_Pl = {
    1000: "日本海北部及びオホーツク海南部",
    1010: "サハリン東方海上",
    1020: "サハリン西方海上",
    1030: "網走沖",
    1040: "宗谷海峡",
    1050: "北海道西方海上",
    1100: "北海道南方及び東方海上",
    1110: "北海道東方海上",
    1120: "釧路沖",
    1130: "日高沖",
    1140: "津軽海峡",
    1150: "檜山津軽沖",
    2000: "三陸沖",
    2010: "三陸沖東部",
    2020: "三陸沖西部",
    3000: "関東海域",
    3010: "関東海域北部",
    3020: "関東海域南部",
    3100: "日本海中部",
    3110: "沿海州南部沖",
    3120: "秋田沖",
    3130: "佐渡沖",
    3140: "能登沖",
    3200: "東海海域",
    3210: "東海海域東部",
    3220: "東海海域西部",
    3230: "東海海域南部",
    4000: "四国沖及び瀬戸内海",
    4010: "瀬戸内海",
    4020: "四国沖北部",
    4030: "四国沖南部",
    4100: "日本海西部",
    4110: "日本海北西部",
    4120: "山陰沖東部及び若狭湾付近",
    4130: "山陰沖西部",
    5000: "対馬海峡",
    5100: "九州西方海上",
    5110: "済州島西海上",
    5120: "長崎西海上",
    5130: "女島南西海上",
    5200: "九州南方海上及び日向灘",
    5210: "日向灘",
    5220: "鹿児島海域",
    5230: "奄美海域",
    6000: "沖縄海域",
    6010: "東シナ海南部",
    6020: "沖縄東方海上",
    6030: "沖縄南方海上",
    10000: "その他の地方海上予報区",
}

def get_mt43_marine_Pl(Pl):

#----------
    return matchtable_mt43_marine_Pl.get(Pl, "地方海上予報区(コード番号：NNNN)")

#---------

def mt43_earthquakeearly_dec(binary_message):
    #Extract data from binary message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    LgL1 = int(binary_message[26:29],2)
    LgU1 = int(binary_message[29:32],2)
    Co = [0]*3
    for i in range(0,3):
        Co[i] = int(binary_message[32+i*9:41+i*9],2)
    OtD1 = int(binary_message[59:64],2)
    OtH1 = int(binary_message[64:69],2)
    OtM1 = int(binary_message[69:75],2)
    De = int(binary_message[75:84],2)
    Ma = int(binary_message[84:91],2)
    Ep = int(binary_message[91:102],2)
    Ll = int(binary_message[102:105],2)
    Ul = int(binary_message[105:109],2)
    Pl = [0]*80
    for i in range(0,80):
        Pl[i] = int(binary_message[109+i*1:110+i*1],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)
    #Decode the message
    message = ''
    message += '防災気象情報(緊急地震速報)\n緊急地震速報\n'
    for i in range(0,3):
        message += get_mt43_Co(Co[i])
        message += '\n'
    message += "\n"
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n"
    message += "震央地名：" + get_mt43_Ep(Ep) +"\n"
    message += "地震発生時刻：" + str(OtD1)+ " 日 " + str(OtH1) + " 時 " + str(OtM1) + " 分\n"
    message += "深さ(km)：" + str(De) + "\n"
    message += "マグニチュード：" + str(Ma) + "\n"
    message += "震度(下限)：" + get_mt43_earthquakeearly_L1(Ll) + "\n"
    message += "震度(上限)：" + get_mt43_earthquakeearly_U1(Ul) + "\n"
    message += "長周期地震動階級（下限）：" + get_mt43_earthquakeearly_LgU1(LgL1) + "\n"
    message += "長周期地震動階級（上限）：" + get_mt43_earthquakeearly_LgU1(LgU1) + "\n"
    for i in range(0,80):
        message += get_mt43_earthquakeearly_Pl(Pl[i])
        message += "\n"
    return message

def mt43_hypocenter_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Co = [0] * 3    
    for i in range(0,3):
        Co[i] = int(binary_message[32+i*9:41+i*9],2)
    OtD1 = int(binary_message[59:64],2)
    OtH1 = int(binary_message[64:69],2)
    OtM1 = int(binary_message[69:75],2)
    De = int(binary_message[75:84],2)
    Ma = int(binary_message[84:91],2)
    Ep = int(binary_message[91:101],2)
    LatNs = int(binary_message[101:102],2)
    LatD = int(binary_message[102:109],2)
    LatM = int(binary_message[109:115],2)
    LatS = int(binary_message[115:121],2)
    LonEw = int(binary_message[121:122],2)
    LonD = int(binary_message[122:130],2)
    LonM = int(binary_message[130:136],2)
    LonS = int(binary_message[136:142],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)
    #Decode the message
    message = ''
    message += "防災気象情報(震源)\n"
    message += str(OtD1) + " 日 " + str(OtH1) + " 時 " + str(OtM1) + " 分ころ、地震がありました。\n"
    for i in range(0,3):
        message += get_mt43_Co(Co[i]) + "\n"
    message += '\n'
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n"
    message += '\n'
    message += "震央地名：" + get_mt43_Ep(Ep) + "\n"
    message += "緯度・経度：" + str(LatNs) + " " + str(LatD) + " 度 " + str(LatM) + " 分 " + str(LatS) + " 秒 " + str(LonEw) + " " + str(LonD) + " 度 " + str(LonM) + " 分 " + str(LonS) + " 秒\n"
    message += "深さ(km)：" + str(De) + "\n"
    message += "マグニチュード：" + str(Ma) + "\n"

    return message

def mt43_seismic_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    OtD1 = int(binary_message[32:37],2)
    OtH1 = int(binary_message[37:42],2)
    OtM1 = int(binary_message[42:48],2)
    Es = [0]*16
    Pl = [0]*16
    for i in range(0,16):
        Es[i] = int(binary_message[48+9*i:51+9*i],2)
        Pl[i] = int(binary_message[51+9*i:57+9*i],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)
    #Decode the message
    message = ''
    message += "防災気象情報(震度)\n"
    message += str(OtD1) + " 日 " + str(OtH1) + " 時 " + str(OtM1) + " 分ころ、地震による強い揺れを感じました。\n"
    message += '\n'
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n"
    message += "\n"
    for i in range(0,16):
        if Es[i] != 0 and Pl[i] != 0:
            message += "震度：" + get_mt43_seismic_Es(Es[i]) + "\n"
            message += get_mt43_seismic_Pl(Pl[i]) + "\n"
        
    return message

def mt43_nankai_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Is = int(binary_message[32:36],2)
    Te = [0]*18
    for i in range(0,18):
        Te[i] = int(binary_message[36+8*i:44+8*i],2)
    Pn = int(binary_message[180:186],2)
    Pm = int(binary_message[186:192],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2) 
    #Decode the message
    message = ''
    message += "防災気象情報(南海トラフ地震)\n南海トラフ地震に関連する情報が発表されました\n\n"
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n"
    message += "地震関連情報：" + get_mt43_nankai_Is(Is) + "\n"
    for i in range(0, 18):
        if Te[i] != 0:
            message += chr(Te).encode('utf-8') + "\n"
    return message

def mt43_tsunami_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Co = [0]*3
    for i in range(0,3):
        Co[i] = int(binary_message[32+9*i:41+9*i],2)
    Dw = int(binary_message[59:63],2)
    TaD1 = [0]*5
    TaH1 = [0]*5
    TaM1 = [0]*5
    Th = [0]*5
    Pl = [0]*5
    for i in range(0,5):
        TaD1[i] = int(binary_message[63+26*i:64+26*i],2)
        TaH1[i] = int(binary_message[64+26*i:69+26*i],2)
        TaM1[i] = int(binary_message[69+26*i:75+26*i],2)
        Th[i] = int(binary_message[75+26*i:79+26*i],2)
        Pl[i] = int(binary_message[79+26*i:89+26*i])
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)     
    #Decode the message

    message = ''
    message += "防災気象情報(津波)\n"
    message += get_mt43_tsunami_Dw(Dw) + " を発表しました。\n"
    for i in range(0,3):
        message += get_mt43_Co(Co[i]) + "\n"
    message += '\n'
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n"
    for i in range(0,5) :
        if TaD1[i] != 0 and TaH1[i] != 0 and TaM1[i] != 0 and Th[i] != 0 and Pl[i] != 0:
            message += "津波到達予想時刻：" + str(TaD1) + " 日 " + str(TaH1) + " 時 " + str(TaM1) + " 分\n"
            message += "津波の高さ：" + get_mt43_tsunami_Th(Th[i]) + '\n'
            message += get_mt43_tsunami_Pl(Pl[i]) + '\n'
            
    return message

def mt43_nwptsunami_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Tp = int(binary_message[32:35],2)
    TaD1 = [0]*5
    TaH1 = [0]*5
    TaM1 = [0]*5
    Th = [0]*5
    Pl = [0]*5
    for i in range(0,5):
        TaD1[i] = int(binary_message[35+28*i:36+28*i],2)
        TaH1[i] = int(binary_message[36+28*i:41+28*i],2)
        TaM1[i] = int(binary_message[41+28*i:47+28*i],2)
        Th[i] = int(binary_message[47+28*i:56+28*i],2)
        Pl[i] = int(binary_message[56+28*i:63+28*i],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)       
    #Decode the message    
    message = ''
    message += "防災気象情報(北西太平洋津波)\n北西太平洋津波情報を発表しました。\n"
    message += get_mt43_nwptsunami_Tp(Tp) + "\n\n"
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n\n"
    for i in range(0,5) :
        if TaD1[i] != 0 and TaH1[i] != 0 and TaM1[i] != 0 and Th[i] != 0 and Pl[i] != 0:
            message += "津波到達予想時刻：" + str(TaD1) + " 日 " + str(TaH1) + " 時 " + str(TaM1) + " 分\n"
            message += "津波の高さ：" + get_mt43_tsunami_Th(Th[i]) + '\n\n'
            message += get_mt43_tsunami_Pl(Pl[i]) + '\n'
    return message

def mt43_volcano_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Du = int(binary_message[29:32],2)
    TdD1 = int(binary_message[32:37],2)
    TdH1 = int(binary_message[37:42],2)
    TdM1 = int(binary_message[42:48],2)
    Dw = int(binary_message[48:55],2)
    Vo = int(binary_message[55:67],2)
    Pl = [0] * 5 
    for i in range (0, 5):
        Pl[i] = int(binary_message[67+23*i:90+23*i],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)     
    #Decode the message    
    message = ''
    message += "防災気象情報(火山)\n火山に関連する情報をお知らせします。\n\n"
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n\n"
    message += "火山名：" + get_mt43_volcano_Vo(Vo) + "\n"
    message += "日時：" + str(TdD1) + " 日 " + str(TdH1) + " 時 " + str(TdM1) + " 分\n"
    message += "現象：" + get_mt43_volcano_Dw(Dw) + '\n'
    message += '\n'
    for i in range(0,5):
        if Pl[i] != 0:
            message += get_mt43_volcano_Pl(Pl[i]) + "\n"
    return message

def mt43_ashfall_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    TdD1 = int(binary_message[32:37],2)
    TdH1 = int(binary_message[37:42],2)
    TdM1 = int(binary_message[42:48],2)
    Dw1 = int(binary_message[48:50],2)
    Vo = int(binary_message[50:62],2)
    Ho = [0]*4
    Dw2 = [0]*4
    Pl = [0]*4
    for i in range(0,4):
        Ho[i] = int(binary_message[62+29*i:65+29*i],2)
        Dw2[i] = int(binary_message[65+29*i:68+29*i],2)
        Pl[i] = int(binary_message[68+29*i:91+29*i],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)    
    #Decode the message    
    message = ''
    message += "防災気象情報(降灰)\n降灰に関連する情報をお知らせします。\n\n"
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n\n"
    #message += get_mt43_ashfall_Dw1(Dw1) + "\n"
    message += "火山名：" + get_mt43_volcano_Vo(Vo) + '\n'
    message += "日時：" + str(TdD1) + " 日 " + str(TdH1) + " 時 " + str(TdM1) + " 分\n\n"
    for i in range(0,4):
        message += "基点時刻からの時間：" + str(Ho[i]) + " 時間\n"
        message += "現象：" + get_mt43_ashfall_Dw2(Dw2[i]) + "\n"
        message += get_mt43_volcano_Pl(Pl[i]) + '\n'
    return message

def mt43_weather_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Ar = int(binary_message[32:35],2)
    Ww = [0]*6
    Pl = [0]*6
    for i in range(0,6):
        Ww[i] = int(binary_message[35+24*i:40+24*i],2)
        Pl[i] = int(binary_message[40+24*i:59+24*i],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)        
    #Decode the message    
    message = ''
    message += "防災気象情報(気象)\n"
    for i in range(0,6):
        if Ww[i] != 0 and Pl[i] != 0:
            message += get_mt43_weather_Ww(Ww[i])+" を "+get_mt43_weather_Ar(Ar)+" しました。\n\n"
            message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n\n"
            message += get_mt43_weather_Pl(Pl[i]) + '\n'
    return message

def mt43_flood_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    Lv = [0]*3
    Pl = [0]*3
    for i in range(0,3):
        Lv[i] = int(binary_message[32+44*i:36+44*i],2)
        Pl[i] = int(binary_message[36+44*i:76+44*i],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)      
    #Decode the message    
    message = ''
    message += '防災気象情報(洪水)\n'
    for i in range(0,3):
        message += get_mt43_flood_Lv(Lv[i]) + '\n'
        message += "氾濫注意水位・流量に到達しました。\n\n"
        message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n\n"
        message += get_mt43_flood_Pl(Pl[i])
    return message

def mt43_typhoon_dec(binary_message):
    #Extract the message
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22],2)
    BtD1 = int(binary_message[32:37],2)
    BtH1 = int(binary_message[37:42],2)
    BtM1 = int(binary_message[42:48],2)
    Dt = int(binary_message[48:51],2)
    Du = int(binary_message[59:66],2)
    Tn = int(binary_message[66:73],2)
    Sr = int(binary_message[73:77],2)
    Ic = int(binary_message[77:81],2)
    LatNs = int(binary_message[81:82],2)
    LatD = int(binary_message[82:89],2)
    LatM = int(binary_message[89:95],2)
    LatS = int(binary_message[95:101],2)
    LonEw = int(binary_message[101:102],2)
    LonD = int(binary_message[102:110],2)
    LonM = int(binary_message[110:116],2)
    LonS = int(binary_message[116:122],2)
    Pr = int(binary_message[122:133],2)
    W1 = int(binary_message[133:140],2)
    W2 = int(binary_message[140:147],2)
    Vn = int(binary_message[193:199],2)
    #CRC = int(binary_message[205:229],2)          
    #Decode the message    
    message = ''
    message += "防災気象情報(台風)\n台風解析・予報情報が発表されました。\n\n"
    message += "発表時刻：" + str(AtMo) + " 月 " + str(AtD) + " 日 " + str(AtH) + " 時 " + str(AtMi) + " 分\n\n"    
    message += "台風番号：" + str(Tn) + "\n"
    message += "基点時刻：" + str(BtD1) + str(BtH1) + str(BtM1) + '\n'
    message += "基点時刻分類：" + str(Dt) + '\n'
    message += "情報の基点時刻からの経過時間：" + str(Du) + " 時間後\n"
    message += "大きさ：" + get_mt43_typhoon_Sr(Sr) + '\n'
    message += "強さ：" + get_mt43_typhoon_Ic(Ic) + '\n'
    message += "緯度・経度：" + str(LatNs) + " " + str(LatD) + " 度 " + str(LatM) + " 分 " + str(LatS) + " 秒 " + str(LonEw) + " " + str(LonD) + " 度 " + str(LonM) + " 分 " + str(LonS) + " 秒\n"
    message += "中心気圧：" + str(Pr) + "\n"
    message += '最大風速：' + str(W1) + '\n'
    message += "最大瞬間風速：" + str(W2) + "\n"
    return message 

def mt43_marine_dec(binary_message):
    #Extract the message    
    AtMo = int(binary_message[0:4],2)
    AtD = int(binary_message[4:9],2)
    AtH = int(binary_message[9:14],2)
    AtMi = int(binary_message[14:20],2)
    It = int(binary_message[20:22], 2)
    Dw = [0]*8
    Pl = [0]*8
    for i in range(0,8):
        Dw[i] = int(binary_message[32+i*19:37+i*19],2)
        Pl[i] = int(binary_message[37+i*19:51+i*19],2)
    #print(Dw)
    #print(Pl)
    #Generate the message
    message = ""
    message_header = "防災気象情報(海上)\n海上警報が発表されました。\n\n"
    message_header += "発表時刻："+str(AtMo)+"月"+str(AtD)+"日"+str(AtH)+"時"+str(AtMi)+"分\n\n"
    message += message_header
    for i in range(0,8):
        if not(Dw[i] == 0 and Pl[i] == 0):            
            message += "警報等情報要素：" + get_mt43_marine_Dw(Dw[i]) + "\n"
            message += get_mt43_marine_Pl(Pl[i]) + "\n"
            #message += "------\n"

    return message
    